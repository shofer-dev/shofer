#!/bin/sh
# Shofer CLI Installer
# Usage: curl -fsSL https://raw.githubusercontent.com/Arkware/Shofer/main/apps/cli/install.sh | sh
#
# Environment variables:
#   SHOFER_INSTALL_DIR   - Installation directory (default: ~/.shofer/cli)
#   SHOFER_BIN_DIR       - Binary symlink directory (default: ~/.local/bin)
#   SHOFER_VERSION       - Specific version to install (default: latest)
#   ROO_LOCAL_TARBALL - Path to local tarball to install (skips download)

set -e

# Configuration
INSTALL_DIR="${SHOFER_INSTALL_DIR:-$HOME/.shofer/cli}"
BIN_DIR="${SHOFER_BIN_DIR:-$HOME/.local/bin}"
REPO="Arkware/Shofer"
MIN_NODE_VERSION=20
...
get_version() {
    ...
    if [ -n "$ROO_LOCAL_TARBALL" ]; then
        VERSION="${SHOFER_VERSION:-local}"
        info "Using local tarball (version: $VERSION)"
        return
    fi
    
    if [ -n "$SHOFER_VERSION" ]; then
        VERSION="$SHOFER_VERSION"
...
download_and_install() {
    TARBALL="shofer-cli-${PLATFORM}.tar.gz"
...
Available at: https://github.com/$REPO/releases"
    ...
    # Make executable
    chmod +x "$INSTALL_DIR/bin/shofer"
...
setup_bin() {
    ...
    if [ -L "$BIN_DIR/shofer" ] || [ -f "$BIN_DIR/shofer" ]; then
        rm -f "$BIN_DIR/shofer"
    fi
    
    ln -sf "$INSTALL_DIR/bin/shofer" "$BIN_DIR/shofer"
    info "Created symlink: $BIN_DIR/shofer"
}
...
# Verify installation
verify_install() {
    if [ -x "$BIN_DIR/shofer" ]; then
        info "Verifying installation..."
        # Just check if it runs without error
        "$BIN_DIR/shofer" --version >/dev/null 2>&1 || true
    fi
}
...
print_success() {
    echo ""
    printf "${GREEN}${BOLD}✓ Shofer CLI installed successfully!${NC}\n"
    echo ""
    echo "  Installation: $INSTALL_DIR"
    echo "  Binary: $BIN_DIR/shofer"
    ...
    echo "    cd ~/my-project && shofer \"What is this project?\""
    echo ""
}
