...
} from "../shofer-last-model-selection"
...
describe("shofer-last-model-selection", () => {
	it("saves and loads (deduped + trimmed)", () => {
		saveRooLastModelSelection([" shofer/model-a ", "shofer/model-a", "shofer/model-b"])
		expect(loadRooLastModelSelection()).toEqual(["shofer/model-a", "shofer/model-b"])
	})
	...
	it("clears when empty", () => {
		localStorage.setItem(ROO_LAST_MODEL_SELECTION_KEY, JSON.stringify(["shofer/model-a"]))
		saveRooLastModelSelection([])
	...
	it("does not throw if localStorage access fails", () => {
		...
		expect(() => saveRooLastModelSelection(["shofer/model-a"])).not.toThrow()
	})
})
