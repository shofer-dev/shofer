...
import { getGitSha, copyPaths, copyLocales, copyWasms, generatePackageJson } from "@shofer/build"
...
async function main() {
	...
	const buildOptions = {
		...
		platform: "node",
		define: {
			"process.env.PKG_NAME": '"shofer-nightly"',
			"process.env.PKG_VERSION": `"${overrideJson.version}"`,
			"process.env.PKG_OUTPUT_CHANNEL": '"Shofer-Nightly"',
