...
- **`sidebar-panel` UI region** is mounted as a collapsible drawer in the chat view, so a plugin (e.g. Live Memory) can show a live panel there. Renders nothing when no plugin contributes one. Live Memory also restores its **chat-input-bar status badge** (agent state dot) as a `chat-input-toolbar` contribution.
...
- **WorkflowView tabs**: Events, Tree (task hierarchy), Sequence, State, **Stats**, and **Logs**. The per-round **topology** graph is now rendered inline in the Events feed (a Mermaid snapshot under each round headline) instead of in a separate tab. The Stats tab shows an active-time donut + per-tool/MCP breakdown aggregated across the whole tree; the Logs tab adds free-text and per-severity filtering. A dedicated **WorkflowHeader** shows whole-tree cost & tokens, a live last-event line, and a shimmer while the flow runs.
