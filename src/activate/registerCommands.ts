...
const getCommandsMap = ({ context, outputChannel, provider }: RegisterCommandOptions): Record<CommandId, any> => ({
	...
	welcomeButtonClicked: () => {
		...
		TelemetryService.instance.captureTitleButtonClicked("welcome")

		visibleProvider.postMessageToWebview({ type: "action", action: "welcomeButtonClicked" })
	},
	tasksButtonClicked: () => {
		const visibleProvider = getVisibleProviderOrLog(outputChannel)
...
export const openShoferInNewTab = async ({ context, outputChannel }: Omit<RegisterCommandOptions, "provider">) => {
	...
	// Shofer Workers L2/D: the editor-tab provider shares the same process-wide
	// WorkerRegistry singleton the sidebar constructed, so this webview also receives
