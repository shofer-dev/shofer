...
beforeEach(async () => {
	tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "live-memory-store-"))
})
...
describe("ConversationStore", () => {
	...
	it("places the file at <storage>/shofer-live-memory-<hash>.json", async () => {
		const store = new ConversationStore(tmpDir, tmpDir)
		expect(store.filePath).toMatch(/shofer-live-memory-[0-9a-f]{16}\.json$/)
	})
})
