...
export async function regexGrepSearch(
	...
	// Filter results using ShoferIgnoreController if provided
	const filteredResults = shoferIgnoreController
		? results.filter((result) => shoferIgnoreController.validateAccess(result.file))
		: results
