...
describe("ShoferConfigService", () => {
	...
	describe("getRooDirectoriesForCwd", () => {
		it("should return directories for given cwd", () => {
			...
			expect(result).toEqual([path.join("/mock/home", ".shofer"), path.join(cwd, ".shofer")])
		})
	})
	...
	describe("loadConfiguration", () => {
		...
		it("should use correct file paths", async () => {
			...
			expect(mockReadFile).toHaveBeenCalledWith(path.join("/mock/home", ".shofer", "rules/rules.md"), "utf-8")
			expect(mockReadFile).toHaveBeenCalledWith(path.join("/project/path", ".shofer", "rules/rules.md"), "utf-8")
		})
	})
	...
	describe("discoverSubfolderRooDirectories", () => {
		...
		it("should discover .shofer directories from subfolders", async () => {
			...
			expect(result).toEqual([
				path.join("/project/path", "package-a", ".shofer"),
				path.join("/project/path", "package-b", ".shofer"),
			])
		})
		...
		it("should sort discovered directories alphabetically", async () => {
			...
			expect(result).toEqual([
				...
				path.join("/project/path", "mango", ".shofer"),
		...
		it("should handle nested subdirectories", async () => {
			...
			expect(result).toEqual([
				path.join("/project/path", "packages/core", ".shofer"),
				path.join("/project/path", "packages/utils", ".shofer"),
			])
		})
		...
		it("should discover .shofer directories with any content", async () => {
			...
			expect(result).toEqual([
				path.join("/project/path", "package-a", ".shofer"),
				path.join("/project/path", "package-b", ".shofer"),
				path.join("/project/path", "package-c", ".shofer"),
				path.join("/project/path", "package-d", ".shofer"),
			])
		})
	})
	...
	describe("getAllRooDirectoriesForCwd", () => {
		...
		it("should return only global and project when no subfolders", async () => {
			...
			expect(result).toEqual([path.join("/mock/home", ".shofer"), path.join("/project/path", ".shofer")])
		})
