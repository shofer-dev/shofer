// npx vitest run api/providers/__tests__/shofer.spec.ts

import { Anthropic } from "@anthropic-ai/sdk"
import { shoferDefaultModelId } from "@shofer/types"
...
vitest.mock("../../../i18n", () => ({
	t: vitest.fn((key: string) => {
		if (key === "common:errors.shofer.authenticationRequired") {
			return "Authentication required for Shofer Cloud"
		}
		return key
	}),
}))
			...
			expect(mockCreate).toHaveBeenCalledWith(
				...
				expect.objectContaining({
					headers: expect.objectContaining({
						"X-Shofer-App-Version": expect.any(String),
					}),
				}),
			)
		})
	...
	describe("completePrompt", () => {
		...
		it("should handle API errors", async () => {
			...
			await expect(handler.completePrompt("Test prompt")).rejects.toThrow(
				"Shofer Cloud completion error: API Error",
			)
		})
