...
import { shoferDefaultModelId, getApiProtocol, type ImageGenerationApiMethod } from "@shofer/types"
...
// Extend OpenAI's CompletionUsage to include Shofer specific fields
interface RooUsage extends OpenAI.CompletionUsage {
	cache_creation_input_tokens?: number
	cost?: number
}

// Add custom interface for Shofer params to support reasoning
...
export class RooHandler extends BaseOpenAiCompatibleProvider<string> {
	...
	constructor(options: ApiHandlerOptions) {
		...
		let baseURL = process.env.SHOFER_PROVIDER_URL ?? "https://api.shofer.com/proxy"
		...
		super({
			...options,
			providerName: "Shofer Cloud",
			baseURL, // Already has /v1 suffix
			apiKey: sessionToken,
			defaultProviderModelId: shoferDefaultModelId,
			providerModels: {},
		})
	...
	protected override createStream(
		...
		// Get Shofer-specific reasoning parameters
	...
	override async *createMessage(
		...
		try {
			...
			const headers: Record<string, string> = {
				"X-Shofer-App-Version": Package.version,
			}

			if (metadata?.taskId) {
				headers["X-Shofer-Task-ID"] = metadata.taskId
			}
			...
			if (lastUsage) {
				...
				const apiProtocol = getApiProtocol("shofer", modelId)
	...
	override getModel() {
		const modelId = this.options.apiModelId || shoferDefaultModelId

		// Get models from shared cache (settings are already applied by the fetcher)
		const models = getModelsFromCache("shofer") || {}
	...
	/**
	 * Generate an image using Shofer Cloud's image generation API
	...
	async generateImage(
		...
		if (!sessionToken || sessionToken === "unauthenticated") {
			return {
				success: false,
				error: t("tools:generateImage.shofer.authRequired"),
			}
		}
