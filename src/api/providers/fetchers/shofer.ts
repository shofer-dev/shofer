import { RooModelsResponseSchema, type ModelInfo, type ModelRecord } from "@shofer/types"
...
/**
 * Fetches available models from the Shofer Cloud provider
 *
 * @param baseUrl The base URL of the Shofer Cloud provider
 * @param apiKey The API key (session token) for the Shofer Cloud provider
...
export async function getRooModels(baseUrl: string, apiKey?: string): Promise<ModelRecord> {
	...
	try {
		...
		try {
			...
			if (!parsed.success) {
				console.error("Error fetching Shofer Cloud models: Unexpected response format", data)
				console.error("Validation errors:", parsed.error.format())
				throw new Error("Failed to fetch Shofer Cloud models: Unexpected response format.")
			}
		...
		console.error("[getRooModels] Error fetching Shofer Cloud models:", {
		...
		if (error.name === "AbortError") {
			throw new Error("Failed to fetch Shofer Cloud models: Request timed out after 10 seconds.")
		}

		// Handle fetch errors
		if (error.message?.includes("HTTP")) {
			throw new Error(`Failed to fetch Shofer Cloud models: ${error.message}. Check base URL and API key.`)
		}

		// Handle network errors
		if (error instanceof TypeError) {
			throw new Error(
				"Failed to fetch Shofer Cloud models: No response from server. Check Shofer Cloud server status and base URL.",
			)
		}

		throw new Error(`Failed to fetch Shofer Cloud models: ${error.message || "An unknown error occurred."}`)
	}
}
