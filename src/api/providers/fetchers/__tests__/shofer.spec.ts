...
describe("getRooModels", () => {
	const baseUrl = "https://api.shofer.com/proxy"
	...
	it("should fetch and parse models successfully", async () => {
		...
		expect(mockFetch).toHaveBeenCalledWith(
			"https://api.shofer.com/proxy/v1/models",
	...
	it("should handle HTTP errors", async () => {
		...
		await expect(getRooModels(baseUrl, apiKey)).rejects.toThrow(
			"Failed to fetch Shofer Cloud models: HTTP 401: Unauthorized",
		)
	})
	...
	it("should handle timeout", async () => {
		...
		await expect(getRooModels(baseUrl, apiKey)).rejects.toThrow(
			"Failed to fetch Shofer Cloud models: Request timed out",
		)
	})
	...
	it("should handle invalid response format", async () => {
		...
		await expect(getRooModels(baseUrl, apiKey)).rejects.toThrow(
			"Failed to fetch Shofer Cloud models: Unexpected response format",
		)
	})
	...
	it("should normalize base URL correctly", async () => {
		...
		await getRooModels("https://api.shofer.com/proxy/v1", apiKey)

		expect(mockFetch).toHaveBeenCalledWith("https://api.shofer.com/proxy/v1/models", expect.any(Object))
	})
	...
	it("should handle network errors", async () => {
		...
		await expect(getRooModels(baseUrl, apiKey)).rejects.toThrow(
			"Failed to fetch Shofer Cloud models: No response from server",
		)
	})
	...
	it("should apply highest versionedSettings for nightly builds (package name)", async () => {
		...
		;(Package as { name: string }).name = "shofer-nightly"
