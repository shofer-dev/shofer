// npx vitest run __tests__/removeShoferFromStack-delegation.spec.ts

import { describe, it, expect, vi } from "vitest"
import { ShoferProvider } from "../core/webview/ShoferProvider"

describe("ShoferProvider.removeShoferFromStack() delegation awareness", () => {
	/**
	...
	it("repairs parent metadata (delegated → active) when a delegated child is removed", async () => {
		...
		await (ShoferProvider.prototype as any).removeShoferFromStack.call(provider)

		// Stack should be empty after pop
		expect(provider.shoferStack).toHaveLength(0)
