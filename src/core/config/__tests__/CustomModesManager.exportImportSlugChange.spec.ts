...
describe("CustomModesManager - Export/Import with Slug Changes", () => {
	...
	describe("Import with Slug Changes", () => {
		it("should place files in rules-{new-slug} folder when slug is changed", async () => {
			...
			expect(rule1Path).toContain(path.join(".shofer", "rules-new-slug-name", "rule1.md"))
			expect(rule2Path).toContain(path.join(".shofer", "rules-new-slug-name", "subfolder", "rule2.md"))
		...
		it("should handle mixed format paths correctly", async () => {
			...
			expect(oldFormatPath).toContain(path.join(".shofer", "rules-mixed-mode", "old-format.md"))
			expect(newFormatPath).toContain(path.join(".shofer", "rules-mixed-mode", "new-format.md"))
			expect(nestedPath).toContain(path.join(".shofer", "rules-mixed-mode", "nested", "file.md"))
		})
	})
