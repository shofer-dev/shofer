...
/**
 * Handles the requestLoadedSkills message — returns loaded skills for the current task.
 * Returns name and full path for each loaded skill.
 */
export async function handleRequestLoadedSkills(provider: ClineProvider): Promise<Record<string, string>> {
	try {
		const currentTask = provider.getCurrentTask()
		const loadedSkills: Record<string, string> = {}
		if (currentTask?.loadedSkills) {
			for (const [name, skillPath] of currentTask.loadedSkills) {
				loadedSkills[name] = skillPath
			}
		}
		await provider.postMessageToWebview({ type: "loadedSkills", loadedSkills })
		return loadedSkills
	} catch (error) {
		provider.log(`Error fetching loaded skills: ${JSON.stringify(error, Object.getOwnPropertyNames(error), 2)}`)
		await provider.postMessageToWebview({ type: "loadedSkills", loadedSkills: {} })
		return {}
	}
}
...
/**
 * Handles the searchSkills message — searches skill SKILL.md files for keywords.
 * Uses grep (or codebase_search if RAG is enabled) scoped to .roo/skills directories.
 */
export async function handleSearchSkills(
	provider: ClineProvider,
	query: string,
): Promise<{ name: string; path: string; matches: string[] }[]> {
	try {
		const skillsManager = provider.getSkillsManager()
		if (!skillsManager) {
			return []
		...
		const allSkills = skillsManager.getSkillsMetadata()
		if (allSkills.length === 0 || !query?.trim()) {
			return []
		}

		const results: { name: string; path: string; matches: string[] }[] = []
			...
			try {
				...
				for (let i = 0; i < lines.length; i++) {
					if (lines[i].toLowerCase().includes(searchTerm)) {
						// Include up to 120 chars of context
						const trimmed = lines[i].trim()
						matches.push(trimmed.length > 120 ? trimmed.substring(0, 120) + "..." : trimmed)
					}
				}

				if (matches.length > 0) {
					results.push({ name: skill.name, path: skillFile, matches })
				}
			} catch {
