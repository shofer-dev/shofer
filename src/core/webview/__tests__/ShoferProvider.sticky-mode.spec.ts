// npx vitest core/webview/__tests__/ShoferProvider.sticky-mode.spec.ts
...
// constructs Task internally via the @shofer/core barrel, and this test creates Task
// directly through the same barrel — so the Task mock lives here (formerly
// vi.mock("../../task/Task")). Keeping it on the old relative path no longer intercepts.
...
describe("ShoferProvider - Sticky Mode", () => {
	...
	describe("Integration with new_task tool", () => {
		it("should preserve parent task mode when creating subtasks", async () => {
			...
			await provider.addShoferToStack(subtask)
	...
	describe("Error handling", () => {
		it("should handle errors gracefully when saving mode fails", async () => {
			...
			vi.spyOn(mockTask as any, "saveShoferMessages").mockRejectedValue(new Error("Save failed"))
	...
	describe("Concurrent mode switches", () => {
		...
		it("should handle mode switches during task save operations", async () => {
			...
			const mockTask = {
				...
				_taskMode: "code",
				emit: vi.fn(),
				saveShoferMessages: vi.fn().mockImplementation(async () => {
					// Simulate slow save
		...
		it("should handle errors during mode switch gracefully", async () => {
			...
			const mockTask = {
				...
				emit: vi.fn().mockImplementation((event) => {
					...
					// The first call is for TaskFocused in addShoferToStack
	...
	describe("Multiple tasks switching modes simultaneously", () => {
		it("should handle multiple tasks switching modes independently", async () => {
			...
			await provider.addShoferToStack(task2 as any)
			await provider.addShoferToStack(task3 as any)
	...
	describe("Task initialization timing edge cases", () => {
		...
		it("should handle rapid task switches during mode changes", async () => {
			...
			for (const task of tasks) {
				await provider.addShoferToStack(task as any)
			}
