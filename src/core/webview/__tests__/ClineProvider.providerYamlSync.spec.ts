// npx vitest run core/webview/__tests__/ClineProvider.providerYamlSync.spec.ts
 ...
 *       * targets the project (`.roomodes`) file when a workspace is open,
...
describe("ClineProvider - custom-mode YAML provider sync", () => {
	...
	describe("syncCustomModeProviderToYaml", () => {
		...
		it("writes to project (.roomodes) when a workspace is open", async () => {
			const { getModeBySlug } = await import("../../../shared/modes")
			vi.mocked(getModeBySlug).mockReturnValue({
				slug: "reviewer",
				name: "Reviewer",
