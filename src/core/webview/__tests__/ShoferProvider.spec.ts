// pnpm --filter shofer test core/webview/__tests__/ShoferProvider.spec.ts
...
describe("ShoferProvider", () => {
	...
	test("constructor initializes correctly", () => {
		expect(provider).toBeInstanceOf(ShoferProvider)
		// Since getVisibleInstance returns the last instance where view.visible is true
		// @ts-ignore - accessing private property for testing
		provider.view = mockWebviewView
		expect(ShoferProvider.getVisibleInstance()).toBe(provider)
	})
	...
	test("resolveWebviewView sets up webview correctly in development mode even if local server is not running", async () => {
		...
		expect(mockWebviewView.webview.html).toContain(
			"connect-src vscode-webview://test-csp-source https://openrouter.ai https://api.requesty.ai https://ph.shofer.com",
		)
	...
	test("dispose is idempotent — second call is a no-op", async () => {
		...
		// dispose body runs only once: log "Disposing ShoferProvider..." appears once
		const disposeCalls = (mockOutputChannel.appendLine as ReturnType<typeof vi.fn>).mock.calls.filter(
			([msg]) => typeof msg === "string" && msg.includes("Disposing ShoferProvider..."),
		)
	...
	test("clearTask aborts current task", async () => {
		// Setup Shofer instance with auto-mock from the top of the file
		const mockShofer = new Task(defaultTaskOptions) // Create a new mocked instance

		// add the mock object to the stack
		await provider.addShoferToStack(mockShofer)

		// get the stack size before the abort call
		const stackSizeBeforeAbort = provider.getTaskStackSize()

		// call the removeShoferFromStack method so it will call the current shofer abort and remove it from the stack
	...
	describe("clearTask message handler", () => {
		...
		test("calls clearTask (delegation handled via metadata)", async () => {
			...
			const mockShofer = new Task(defaultTaskOptions)
		...
		test("calls clearTask even with parent task (delegation via metadata)", async () => {
			...
			await provider.addShoferToStack(parentTask)
			await provider.addShoferToStack(childTask)
	...
	test("addShoferToStack adds multiple Shofer instances to the stack", async () => {
		// Setup Shofer instance with auto-mock from the top of the file
		const mockShofer1 = new Task(defaultTaskOptions) // Create a new mocked instance
		const mockShofer2 = new Task(defaultTaskOptions) // Create a new mocked instance
		Object.defineProperty(mockShofer1, "taskId", { value: "test-task-id-1", writable: true })
		Object.defineProperty(mockShofer2, "taskId", { value: "test-task-id-2", writable: true })

		// add Shofer instances to the stack
		await provider.addShoferToStack(mockShofer1)
		await provider.addShoferToStack(mockShofer2)

		// verify shofer instances were added to the stack
		expect(provider.getTaskStackSize()).toBe(2)

		// verify current shofer instance is the last one added
		expect(provider.getCurrentTask()).toBe(mockShofer2)
	})
	...
	test("handles showShoferIgnoredFiles setting", async () => {
		await provider.resolveWebviewView(mockWebviewView)
		const messageHandler = (mockWebviewView.webview.onDidReceiveMessage as any).mock.calls[0][0]

		// Default value should be false
		expect((await provider.getState()).showShoferIgnoredFiles).toBe(false)

		// Test showShoferIgnoredFiles with true
		await messageHandler({ type: "updateSettings", updatedSettings: { showShoferIgnoredFiles: true } })
		expect(mockContext.globalState.update).toHaveBeenCalledWith("showShoferIgnoredFiles", true)
		expect(mockPostMessage).toHaveBeenCalled()
		expect((await provider.getState()).showShoferIgnoredFiles).toBe(true)

		// Test showShoferIgnoredFiles with false
		await messageHandler({ type: "updateSettings", updatedSettings: { showShoferIgnoredFiles: false } })
		expect(mockContext.globalState.update).toHaveBeenCalledWith("showShoferIgnoredFiles", false)
		expect(mockPostMessage).toHaveBeenCalled()
		expect((await provider.getState()).showShoferIgnoredFiles).toBe(false)
	})
	...
	describe("deleteMessage", () => {
		...
		test("handles deletion with confirmation dialog", async () => {
			...
			const mockShofer = new Task(defaultTaskOptions) // Create a new mocked instance
			mockShofer.shoferMessages = mockMessages // Set test-specific messages
			mockShofer.apiConversationHistory = mockApiHistory // Set API history
			await provider.addShoferToStack(mockShofer) // Add the mocked instance to the stack
			...
			expect(mockShofer.overwriteShoferMessages).toHaveBeenCalledWith([
				mockMessages[0],
				mockMessages[1],
				mockMessages[2],
			])
			...
			expect(mockShofer.overwriteApiConversationHistory).toHaveBeenCalledWith([
				mockApiHistory[0],
				mockApiHistory[1],
				mockApiHistory[2],
			])
		...
		test("handles case when no current task exists", async () => {
			...
			// Verify no dialog was shown since there's no current shofer
	...
	describe("editMessage", () => {
		...
		test("handles edit with confirmation dialog", async () => {
			...
			mockShofer.overwriteShoferMessages = vi.fn()
			mockShofer.overwriteApiConversationHistory = vi.fn()
			mockShofer.handleWebviewAskResponse = vi.fn()
...
describe("Project MCP Settings", () => {
	...
	test.skip("handles openProjectMcpSettings message", async () => {
		...
		expect(mockedFs.mkdir).toHaveBeenCalledWith("/test/workspace/.shofer", { recursive: true })

		// Verify file was created with default content
		expect(safeWriteJson).toHaveBeenCalledWith("/test/workspace/.shofer/mcp.json", { mcpServers: {} })

		// Check that openFile was called
		expect(openFileSpy).toHaveBeenCalledWith("/test/workspace/.shofer/mcp.json")
	})
	...
	test.skip("handles openProjectMcpSettings file creation error", async () => {
		...
		expect(vscode.window.showErrorMessage).toHaveBeenCalledWith(
			expect.stringContaining("Failed to create or open .shofer/mcp.json"),
		)
	})
})
...
describe.skip("ContextProxy integration", () => {
	...
	beforeEach(() => {
		...
		provider = new ShoferProvider(mockContext, mockOutputChannel, "sidebar", mockContextProxy)
	})
...
describe("getTelemetryProperties", () => {
	...
	beforeEach(() => {
		...
		mockShofer = new Task(defaultTaskOptions)
		mockShofer.api = {
			getModel: vi.fn().mockReturnValue({
	...
	test("includes model ID from current Shofer instance if available", async () => {
		// Add mock Shofer to stack
		await provider.addShoferToStack(mockShofer)
...
describe("ShoferProvider - Router Models", () => {
	let provider: ShoferProvider
	let mockContext: vscode.ExtensionContext
	let mockOutputChannel: vscode.OutputChannel
	let mockWebviewView: vscode.WebviewView
	...
	test("handles requestRouterModels with individual provider failures", async () => {
		...
		vi.mocked(getModels)
			...
			.mockResolvedValueOnce(mockModels) // shofer success
...
describe("ShoferProvider - Comprehensive Edit/Delete Edge Cases", () => {
	let provider: ShoferProvider
	let mockContext: vscode.ExtensionContext
	let mockOutputChannel: vscode.OutputChannel
	let mockWebviewView: vscode.WebviewView
	...
	describe("Edit Messages with Images and Attachments", () => {
		...
		test("handles editing messages containing images", async () => {
			...
			const mockShofer = new Task(defaultTaskOptions)
			mockShofer.shoferMessages = mockMessages
			mockShofer.apiConversationHistory = [{ ts: 1000 }, { ts: 2000 }, { ts: 3000 }] as any[]
			mockShofer.overwriteShoferMessages = vi.fn()
			mockShofer.overwriteApiConversationHistory = vi.fn()
			mockShofer.submitUserMessage = vi.fn()
			...
			expect(mockShofer.overwriteShoferMessages).toHaveBeenCalledWith([mockMessages[0]])
			expect(mockShofer.overwriteApiConversationHistory).toHaveBeenCalledWith([{ ts: 1000 }])
			// Verify submitUserMessage was called with the edited content
			expect(mockShofer.submitUserMessage).toHaveBeenCalledWith("Edited message with preserved images", [])
		})
		...
		test("handles editing messages with file attachments", async () => {
			...
			expect(mockShofer.overwriteShoferMessages).toHaveBeenCalled()
			expect(mockShofer.submitUserMessage).toHaveBeenCalledWith("Edited message with file attachment", [])
		})
	})
	...
	describe("Network Failure Scenarios", () => {
		...
		test("handles network timeout during edit submission", async () => {
			const mockShofer = new Task(defaultTaskOptions)
			mockShofer.shoferMessages = [
				{ ts: 1000, type: "say", say: "user_feedback", text: "Original message", value: 2000 },
				{ ts: 2000, type: "say", say: "text", text: "AI response" },
			] as ShoferMessage[]
			mockShofer.apiConversationHistory = [{ ts: 1000 }, { ts: 2000 }] as any[]
			mockShofer.overwriteShoferMessages = vi.fn()
			mockShofer.overwriteApiConversationHistory = vi.fn()
			mockShofer.handleWebviewAskResponse = vi.fn().mockRejectedValue(new Error("Network timeout"))
		...
		test("handles connection drops during edit operation", async () => {
			...
			mockShofer.apiConversationHistory = [{ ts: 1000 }, { ts: 2000 }] as any[]
			mockShofer.overwriteShoferMessages = vi.fn().mockRejectedValue(new Error("Connection lost"))
			mockShofer.overwriteApiConversationHistory = vi.fn()
			mockShofer.handleWebviewAskResponse = vi.fn()
	...
	describe("Concurrent Edit Operations", () => {
		...
		test("handles race conditions with simultaneous edits", async () => {
			...
			mockShofer.apiConversationHistory = [{ ts: 1000 }, { ts: 2000 }, { ts: 3000 }, { ts: 4000 }] as any[]
			mockShofer.overwriteShoferMessages = vi.fn()
			mockShofer.overwriteApiConversationHistory = vi.fn()
			mockShofer.handleWebviewAskResponse = vi.fn()
	...
	describe("Edit Permissions and Authorization", () => {
		...
		test("handles edit permission failures", async () => {
			// Mock no current shofer (simulating permission failure)
			...
			// Should not show confirmation dialog when no current shofer
		...
		test("handles authorization failures during edit", async () => {
			...
			mockShofer.apiConversationHistory = [{ ts: 1000 }, { ts: 2000 }] as any[]
			mockShofer.overwriteShoferMessages = vi.fn().mockRejectedValue(new Error("Unauthorized"))
			mockShofer.overwriteApiConversationHistory = vi.fn()
			mockShofer.handleWebviewAskResponse = vi.fn()
		...
		describe("Operations on Deleted or Non-existent Messages", () => {
			...
			test("handles edit operations on deleted messages", async () => {
				...
				mockShofer.apiConversationHistory = [{ ts: 1000 }] as any[]
				mockShofer.overwriteShoferMessages = vi.fn()
				mockShofer.overwriteApiConversationHistory = vi.fn()
				mockShofer.handleWebviewAskResponse = vi.fn()
				...
				expect(mockShofer.overwriteShoferMessages).not.toHaveBeenCalled()
				expect(mockShofer.handleWebviewAskResponse).not.toHaveBeenCalled()
			})
		...
		describe("Resource Cleanup During Failed Operations", () => {
			...
			test("validates proper cleanup during failed edit operations", async () => {
				...
				mockShofer.overwriteShoferMessages = vi.fn().mockImplementation(() => {
					cleanupSpy()
		...
		describe("Large Message Payloads", () => {
			...
			test("handles editing messages with large text content", async () => {
				...
				expect(mockShofer.overwriteShoferMessages).toHaveBeenCalled()
				expect(mockShofer.submitUserMessage).toHaveBeenCalledWith(largeEditedContent, [])
			})
			...
			test("handles deleting messages with large payloads", async () => {
				...
				expect(mockShofer.overwriteShoferMessages).toHaveBeenCalledWith([mockMessages[0], mockMessages[1]])
				expect(mockShofer.overwriteApiConversationHistory).toHaveBeenCalledWith([{ ts: 1000 }, { ts: 2000 }])
			})
		})
		...
		describe("Error Messaging and User Feedback", () => {
			...
			test("handles user cancellation gracefully", async () => {
				...
				expect(mockShofer.overwriteShoferMessages).not.toHaveBeenCalled()
				expect(mockShofer.overwriteApiConversationHistory).not.toHaveBeenCalled()
				expect(mockShofer.handleWebviewAskResponse).not.toHaveBeenCalled()
				expect(vscode.window.showErrorMessage).not.toHaveBeenCalled()
			})
		})
		...
		describe("Edge Cases with Message Timestamps", () => {
			...
			test("handles messages with identical timestamps", async () => {
				...
				mockShofer.apiConversationHistory = [{ ts: 1000 }, { ts: 1000 }, { ts: 1000 }, { ts: 2000 }] as any[]
				mockShofer.overwriteShoferMessages = vi.fn()
				mockShofer.overwriteApiConversationHistory = vi.fn()
			...
			test("handles messages with future timestamps", async () => {
				...
				mockShofer.apiConversationHistory = [
					{ ts: 1000 },
					{ ts: futureTimestamp },
				...
				expect(mockShofer.overwriteShoferMessages).toHaveBeenCalled()
				expect(mockShofer.submitUserMessage).toHaveBeenCalled()
			})
		})
	})
