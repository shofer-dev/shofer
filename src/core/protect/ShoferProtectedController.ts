...
/**
 * Controls write access to Shofer configuration files by enforcing protection patterns.
 * Prevents auto-approved modifications to sensitive Shofer configuration files.
 */
export class ShoferProtectedController {
	private cwd: string
	...
	constructor(cwd: string) {
		...
		this.ignoreInstance.add(ShoferProtectedController.PROTECTED_PATTERNS)
	}
	...
	getProtectionMessage(): string {
		return "This is a Shofer configuration file and requires approval for modifications"
	}
	...
	getInstructions(): string {
		const patterns = ShoferProtectedController.PROTECTED_PATTERNS.join(", ")
		return `# Protected Files\n\n(The following Shofer configuration file patterns are write-protected and always require approval for modifications, regardless of autoapproval settings. When using list_files, you'll notice a ${SHIELD_SYMBOL} next to files that are write-protected.)\n\nProtected patterns: ${patterns}`
	}
	...
	static getProtectedPatterns(): readonly string[] {
		return ShoferProtectedController.PROTECTED_PATTERNS
	}
}
