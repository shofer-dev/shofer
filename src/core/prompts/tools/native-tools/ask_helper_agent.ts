...
import { DEFAULT_HELPER_SOFT_TIMEOUT_SEC, DEFAULT_HELPER_SOFT_RESULT_LENGTH } from "@shofer/types"
...
- softTimeoutSec (number, optional): Soft recommendation in seconds for how long the helper agent should spend on this question (default: ${DEFAULT_HELPER_SOFT_TIMEOUT_SEC}). Embedded in the prompt as guidance — the agent will try to wrap up around that time but is NOT cancelled.
...
const SOFT_TIMEOUT_SEC_PARAMETER_DESCRIPTION = `Soft recommendation in seconds for how long the helper agent should spend on this question (default: ${DEFAULT_HELPER_SOFT_TIMEOUT_SEC}). Embedded in the prompt as guidance; not enforced as cancellation.`
