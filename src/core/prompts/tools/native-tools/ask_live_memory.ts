...
Parameters:
- question (string, required): The question to ask the live memory agent.
- contextFiles (string[], optional): File paths that are relevant to this question. The live memory agent will load these into its context window if they aren't already present.
...
const QUESTION_PARAMETER_DESCRIPTION = `The question to ask the live memory agent`

const CONTEXT_FILES_PARAMETER_DESCRIPTION = `Optional file paths that are relevant to this question. The live memory agent will load these into its context window.`
...
const SOFT_TIMEOUT_SEC_PARAMETER_DESCRIPTION = `Soft recommendation in seconds for how long the live memory agent should spend on this question (default: ${DEFAULT_LIVE_MEMORY_SOFT_TIMEOUT_SEC}). Embedded in the prompt as guidance; not enforced as cancellation.`
