...
describe("ShoferIgnore Response Formatting", () => {
	...
	describe("formatResponse.shoferIgnoreError", () => {
		/**
		 * Tests the error message format for ignored files
		 */
		it("should format error message for ignored files", () => {
			const errorMessage = formatResponse.shoferIgnoreError("secrets/api-keys.json")
		...
		it("should include the file path in the error message", () => {
			...
			for (const testPath of paths) {
				const errorMessage = formatResponse.shoferIgnoreError(testPath)
				const parsed = JSON.parse(errorMessage) as any
	...
	describe("formatResponse.formatFilesList with ShoferIgnoreController", () => {
		/**
		...
		/**
		 * Tests formatFilesList when showShoferIgnoredFiles is set to false
		 */
		it("should hide ignored files when showShoferIgnoredFiles is false", async () => {
			// Create controller
			const controller = new ShoferIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Format with controller and showShoferIgnoredFiles = false
			const result = formatResponse.formatFilesList(
				TEST_CWD,
				files,
				false,
				controller as any,
				false, // showShoferIgnoredFiles = false
			)
		...
		/**
		 * Tests formatFilesList handles truncation correctly with ShoferIgnoreController
		 */
		it("should handle truncation with ShoferIgnoreController", async () => {
			// Create controller
			const controller = new ShoferIgnoreController(TEST_CWD)
			await controller.initialize()
		 ...
		 */
		it("should handle empty file list with ShoferIgnoreController", async () => {
			// Create controller
			const controller = new ShoferIgnoreController(TEST_CWD)
			await controller.initialize()
