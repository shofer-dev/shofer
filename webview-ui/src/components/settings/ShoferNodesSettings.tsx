		...
		useImperativeHandle(
			...
			(): ShoferNodesSettingsRef => ({
				commitNodeBuffers: () => {
					...
					if (policy !== undefined && policy !== (live?.loadBalancer ?? "round-robin")) {
					...
					setPendingLoadBalancer(undefined)
					setPendingDisabled({})
				},
				discardNodeBuffers: () => {
					setPendingLoadBalancer(undefined)
				...
				<Section>
					...
					<div className="flex flex-col gap-2">
						{nodes.map((n) => {
							...
							const effectiveDisabled = pendingDisabled[n.id] ?? Boolean(n.disabled)
							return (
								<div
									key={n.id}
									className={cn(
									...
									<div className="flex items-center gap-2">
										...
										<Button
											...
											title={
												effectiveDisabled
													? "Enable (return to pool)"
													: "Disable (remove from pool)"
											}
											onClick={() => {
												setPendingDisabled((prev) => ({ ...prev, [n.id]: !effectiveDisabled }))
												onNodesDirty?.()
											}}>
											{effectiveDisabled ? (
									...
									<div className="text-xs text-vscode-descriptionForeground break-words pl-4">
										{n.kind === "local"
											? [
													effectiveDisabled ? "disabled" : "running locally",
