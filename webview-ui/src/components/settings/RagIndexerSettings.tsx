...
import { CodeIndexConfigForm, type CodeIndexConfigFormRef } from "./CodeIndexConfigForm"
import {
	Slider,
	AlertDialog,
	AlertDialogAction,
 ...
 */
export interface RagIndexerSettingsRef {
	saveCodeIndexSecrets: () => void
}

export const RagIndexerSettings = forwardRef<RagIndexerSettingsRef, RagIndexerSettingsProps>(
	({ codebaseIndexConfig, setCachedStateField }, ref) => {
		const { t } = useAppTranslation()
		const codeIndexConfigFormRef = useRef<CodeIndexConfigFormRef>(null)

		useImperativeHandle(ref, () => ({
			saveCodeIndexSecrets: () => {
				codeIndexConfigFormRef.current?.saveSecrets()
			},
		}))
			...
			<div>
				...
				<div className="space-y-6">
					...
					<div>
							...
							<SearchableSetting
								...
								<div className="flex items-center gap-2">
									<VSCodeCheckbox
										checked={codeEnabled}
										onChange={(e: any) =>
											updateGitSetting("codebaseIndexEnabled", e.target.checked)
										}>
						...
						<CodeIndexConfigForm ref={codeIndexConfigFormRef} setCachedStateField={setCachedStateField} />
					</div>
						...
						<div className="space-y-4">
							...
							<SearchableSetting
								...
								<VSCodeCheckbox
									checked={gitEnabled}
									onChange={(e: any) =>
										updateGitSetting("codebaseIndexGitEnabled", e.target.checked)
									}>
								...
								<>
									...
									<SearchableSetting
										...
										<VSCodeTextField
											value={gitBranch}
											onInput={(e: any) =>
												updateGitSetting("codebaseIndexGitBranch", e.target.value)
											}
											placeholder="master"
										/>
										<p className="text-xs text-vscode-descriptionForeground mt-1">
