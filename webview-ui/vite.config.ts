...
export default defineConfig(({ mode }) => {
	...
	// TODO: We can use `@shofer/build` to generate `define` once the
	...
	if (mode === "nightly") {
		...
		define["process.env.PKG_OUTPUT_CHANNEL"] = JSON.stringify("Shofer-Nightly")
	}
	...
	return {
		...
		resolve: {
			alias: {
				...
				"@shofer/shared": resolve(__dirname, "../src/shared"),
			},
		},
		build: {
			outDir,
			...
			rollupOptions: {
				...
				// dynamically imported by shofer-config/index.ts, but should never be bundled
