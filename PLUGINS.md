...
| `unqualifiedModes`       | boolean             | **Bundled (first-party) scope only.** Register `contributes.modes` under their authored slugs instead of `<plugin>:<slug>`. Exists for a plugin shipping the platform's own defaults, whose names are a contract (`plugins/builtin-modes/` — the built-in modes must stay `code`, `architect`, …). Ignored for global/project plugins: an unqualified third-party slug could shadow a built-in.               |
