...
export function copyWasms(srcDir: string, distDir: string): void {
	...
	// NOTE: esbuild-wasm WASM binary (~12 MB) is no longer copied. It was
	// a build-time dependency that was never loaded at runtime — zero imports
	// reference esbuild.wasm, wasm_exec_node.js, or bin/esbuild in the
	// extension bundle or workers.  Removing it saves 12+ MB from the VSIX.
}
