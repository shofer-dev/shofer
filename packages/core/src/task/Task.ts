...
export class Task extends EventEmitter<TaskEvents> implements TaskLike {
	...
	/**
	 * This task's native tool-call stream assembler.
	 *
	 * Owned per task, never shared: the wire `index` a provider stamps on each
	 * argument fragment restarts at 0 for every stream, so a process-global
	 * assembler splices two concurrently streaming tasks' tool-call arguments
	 * into one another (see `NativeToolCallParser`'s class docstring). A
	 * headless worker runs many tasks in one process, so "concurrently
	 * streaming" is the normal case there, not an edge one.
	 */
