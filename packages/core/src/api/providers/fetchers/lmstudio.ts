...
import { flushModels } from "./modelCache.js"
...
export const forceFullModelDetailsLoad = async (baseUrl: string, modelId: string): Promise<void> => {
	try {
		...
		if ((error as NodeJS.ErrnoException).code === "ECONNREFUSED") {
			apiLog.warn(`Error connecting to LMStudio at ${baseUrl}`)
		} else {
			apiLog.error(
...
export async function getLMStudioModels(baseUrl = "http://localhost:1234"): Promise<Record<string, ModelInfo>> {
	...
	try {
		...
		try {
			...
			apiLog.warn("Failed to list downloaded models, falling back to loaded models only")
		}
