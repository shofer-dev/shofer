...
import { ApiHandlerOptions } from "../_deps.js"
import { vercelAiGatewayDefaultModelId, VERCEL_AI_GATEWAY_DEFAULT_TEMPERATURE } from "@shofer/types"
...
vitest.mock("../_deps.js", async (importOriginal) => ({
...
describe("VercelAiGatewayHandler", () => {
	...
	it("initializes with correct options", () => {
		...
		expect(OpenAI).toHaveBeenCalledWith({
			...
			defaultHeaders: expect.objectContaining({
				...
				"X-Title": "Shofer",
				"User-Agent": expect.stringContaining("Shofer/"),
			}),
		})
	})
	...
	describe("createMessage", () => {
		...
		it("adds cache breakpoints for supported models", async () => {
			const { addVercelAiGatewayCacheBreakpoints: addCacheBreakpoints } = await import("../_deps.js")
			const handler = new VercelAiGatewayHandler({
				...mockOptions,
