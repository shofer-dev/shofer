import { handleProviderError, handleOpenAIError } from "../error-handler.js"
...
describe("handleOpenAIError (backward compatibility)", () => {
	...
	it("should preserve backward compatibility for existing callers", () => {
		...
		const result = handleOpenAIError(error, "Shofer Cloud")

		expect(result.message).toBe("Shofer Cloud completion error: Authentication failed")
		expect((result as any).status).toBe(401)
	})
})
