...
import { DEFAULT_MAX_PARALLEL_TASKS } from "@shofer/types"
...
export class NewTaskTool extends BaseTool<"new_task"> {
	...
	async execute(params: NewTaskParams, task: Task, callbacks: ToolCallbacks): Promise<void> {
		...
		try {
			...
			const effectiveLimit = maxParallel ?? DEFAULT_MAX_PARALLEL_TASKS
			if (effectiveLimit > 0) {
			...
			if (TelemetryService.hasInstance()) {
				TelemetryService.instance.captureSubtaskSpawned(task.taskId, effectiveMode)
			}

			// Every child is CONCURRENT. There is no foreground mode and no focus
			// steal: a blocked parent could not answer its own child's questions,
			// could not cancel it, and could not coordinate anything else — which is
			// what made the sync path a deadlock waiting to happen. A parent that
			// wants the result calls `wait`; the child's `attempt_completion`
			// delivers it to the parent's mailbox (docs/task_messaging.md).
			...
			// changing the parent's status (it keeps running).
