...
describe("MockWorkspaceConfiguration", () => {
	...
	describe("Runtime Configuration", () => {
		...
		it("should return runtime config value over disk-based values", async () => {
			const config = new MockWorkspaceConfiguration("shofer", context)
			...
			setRuntimeConfig("shofer", "commandExecutionTimeout", 20)
		...
		it("should set and get runtime config values", () => {
			setRuntimeConfig("shofer", "testSetting", "testValue")

			expect(getRuntimeConfig("shofer.testSetting")).toBe("testValue")
		})

		it("should set multiple runtime config values at once", () => {
			setRuntimeConfigValues("shofer", {
				setting1: "value1",
				setting2: 42,
				setting3: true,
			})

			expect(getRuntimeConfig("shofer.setting1")).toBe("value1")
			expect(getRuntimeConfig("shofer.setting2")).toBe(42)
			expect(getRuntimeConfig("shofer.setting3")).toBe(true)
		})
		...
		it("should ignore undefined values in setRuntimeConfigValues", () => {
			...
			expect(getRuntimeConfig("shofer.defined")).toBe("value")
			expect(getRuntimeConfig("shofer.notDefined")).toBeUndefined()
		})

		it("should clear all runtime config values", () => {
			setRuntimeConfig("shofer", "setting1", "value1")
			setRuntimeConfig("shofer", "setting2", "value2")

			clearRuntimeConfig()

			expect(getRuntimeConfig("shofer.setting1")).toBeUndefined()
			expect(getRuntimeConfig("shofer.setting2")).toBeUndefined()
		})
