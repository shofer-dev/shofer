...
import { getClerkBaseUrl, getShoferApiUrl } from "../config.js"
...
describe("WebAuthService", () => {
	...
	beforeEach(() => {
		...
		vi.mocked(getClerkBaseUrl).mockReturnValue("https://clerk.shofer.com")
		vi.mocked(getShoferApiUrl).mockReturnValue("https://api.test.com")

		// Setup utils mock
		vi.mocked(getUserAgent).mockReturnValue("Shofer 1.0.0")
	...
	describe("login", () => {
		...
		it("should use package.json values for redirect URI with default sign-in endpoint", async () => {
			...
			const expectedUrl =
				"https://api.test.com/extension/sign-in?state=746573742d72616e646f6d2d6279746573&auth_redirect=vscode%3A%2F%2FArkware.shofer"
			expect(mockOpenExternal).toHaveBeenCalledWith(
				expect.objectContaining({
					toString: expect.any(Function),
				}),
		...
		it("should use provider signup URL when useProviderSignup is true", async () => {
			...
			const expectedUrl =
				"https://api.test.com/extension/provider-sign-up?state=746573742d72616e646f6d2d6279746573&auth_redirect=vscode%3A%2F%2FArkware.shofer"
			expect(mockOpenExternal).toHaveBeenCalledWith(
				expect.objectContaining({
					toString: expect.any(Function),
				}),
		...
		it("should handle errors during login", async () => {
			...
			await expect(authService.login()).rejects.toThrow("Failed to initiate Shofer Cloud authentication")
			expect(mockLog).toHaveBeenCalledWith("[auth] Error initiating Shofer Cloud auth: Error: Crypto error")
		})
	})
	...
	describe("handleCallback", () => {
		...
		it("should handle invalid parameters", async () => {
			...
			expect(mockShowInfo).toHaveBeenCalledWith("Invalid Shofer Cloud sign in url")
		...
		it("should validate state parameter", async () => {
			...
			await expect(authService.handleCallback("code", "different-state")).rejects.toThrow(
				"Failed to handle Shofer Cloud callback",
			)
		...
		it("should successfully handle valid callback", async () => {
			...
			expect(mockShowInfo).toHaveBeenCalledWith("Successfully authenticated with Shofer Cloud")
		})
		...
		it("should store provider model when provided in callback", async () => {
			...
			expect(mockContext.globalState.update).toHaveBeenCalledWith("shofer-provider-model", "xai/grok-code-fast-1")
			expect(mockContext.globalState.update).toHaveBeenCalledWith("shofer-auth-skip-model", undefined)
		...
		it("should set skip model flag when provider model is NOT provided in callback", async () => {
			...
			expect(mockContext.globalState.update).toHaveBeenCalledWith("shofer-auth-skip-model", true)
	...
	describe("logout", () => {
		...
		it("should clear credentials and call Clerk logout", async () => {
			...
			expect(mockFetch).toHaveBeenCalledWith(
				"https://clerk.shofer.com/v1/client/sessions/test-session/remove",
				expect.objectContaining({
					method: "POST",
			...
			expect(mockShowInfo).toHaveBeenCalledWith("Logged out from Shofer Cloud")
		})
