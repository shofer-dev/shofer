...
import { type TaskEvent, ShoferEventName } from "@shofer/types"
...
export const processTask = async ({
	...
	try {
		...
		await publish({
			eventName: passed ? ShoferEventName.EvalPass : ShoferEventName.EvalFail,
			taskId: task.id,
		})
	} finally {
...
export const processTaskInContainer = async ({
	...
	if (jobToken) {
		baseArgs.push(`-e SHOFER_CLOUD_TOKEN=${jobToken}`)
	}
	...
	const command = `pnpm --filter @shofer/evals cli --taskId ${taskId}`
	logger.info(command)
