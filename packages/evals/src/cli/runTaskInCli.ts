...
import { type ToolUsage, TaskCommandName, ShoferEventName, IpcMessageType } from "@shofer/types"
...
/**
 * Run a task using the Shofer CLI (headless mode).
...
export const runTaskWithCli = async ({ run, task, publish, logger, jobToken }: RunTaskOptions) => {
	...
	const env: Record<string, string> = {
		...
		SHOFER_IPC_SOCKET_PATH: ipcSocketPath,
	}

	if (jobToken) {
		env.SHOFER_CLOUD_TOKEN = jobToken
	}
	...
	const ignoreEventsForBroadcast = [ShoferEventName.Message]
	...
	client.on(IpcMessageType.TaskEvent, async (taskEvent) => {
		...
		// but we still want to capture the shoferTaskId from TaskStarted if we receive it.
		if (eventName === ShoferEventName.TaskStarted) {
			taskStartedAt = Date.now()
			shoferTaskId = payload[0]
			logger.info(`received TaskStarted event, shoferTaskId: ${shoferTaskId}`)
		}
