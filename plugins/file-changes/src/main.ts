/**
 * File Changes — per-task tracking of the files the agent edited, as a first-party
 * Shofer plugin.
 *
 ...
 *
 * Removing the feature is removing this directory: core keeps only the two generic
 * file-edit hooks, which say what happened and to which path, and nothing about
 * panels, baselines or reverts.
 */
...
import type { PluginContext, PluginFileEdit, PluginFileEditResult, ShoferPlugin } from "@shofer/types"
...
const PLUGIN_NAME = "file-changes"
...
function storeFor(ctx: PluginContext, taskId?: string, cwd?: string): FileSnapshotStore | undefined {
	...
	const dir = state.ctx?.storage?.dir ?? ctx.storage?.dir
...
function pushUpdate(ctx: PluginContext, taskId?: string): void {
	...
	state.pushChain = state.pushChain
		...
		.catch((error: unknown) => {
			ctx.host?.log.warn(`[${PLUGIN_NAME}] failed to push the change list: ${String(error)}`)
		})
}
...
async function revertEverything(ctx: PluginContext, confirmed?: boolean): Promise<{ reverted: boolean }> {
	...
	await restoreAll(store, cwd, (relPath, error) =>
		ctx.host?.log.warn(`[${PLUGIN_NAME}] revert failed for ${relPath}: ${String(error)}`),
	)
	pushUpdate(ctx)
	return { reverted: true }
}
...
const plugin: ShoferPlugin = {
	...
	initialize(ctx: PluginContext): void {
		...
		state.stores.clear()
		ctx.host?.log.info("file-changes plugin initialized")
	},
	...
	lifecycle: {
		...
		async onTaskDeleted(info: { taskId: string }, ctx: PluginContext) {
			...
			try {
				...
				ctx.host?.log.warn(`[${PLUGIN_NAME}] failed to remove snapshots for ${info.taskId}: ${String(error)}`)
			}
		},
	},
	...
	/**
	 * The panel's request surface. Everything that reads or mutates this task's
	 * snapshots is answered on the host that owns the task (so a task running on a
	 * remote executor behaves identically); `local:` methods are answered here, because
	 * only this host has the user's editor.
	 */
		...
		switch (method) {
			...
			case "accept-all": {
				...
				await acceptAll(store, cwd, (relPath, error) =>
					ctx.host?.log.warn(`[${PLUGIN_NAME}] accept failed for ${relPath}: ${String(error)}`),
				)
				pushUpdate(ctx)
				return { accepted: true }
			}
