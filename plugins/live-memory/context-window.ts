/**
 * ContextWindow — token accounting + LRU eviction for the Live Memory plugin.
 *
 ...
 * single source of truth for "how full is the window".
 *
 * Token estimation uses the conventional 4-chars-per-token heuristic. This is
 * intentionally cheap; providers' authoritative usage numbers are folded back via the
 * cost ledger after each LLM call.
 *
 ...
 *
 * Truncated tokens are accumulated into the cost ledger so the UI can surface
 * "X tokens evicted this session".
 */
