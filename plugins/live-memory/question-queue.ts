/**
 * QuestionQueue — bounded FIFO queue for Live Memory questions.
 *
 ...
 * Each entry has its own timeout that covers BOTH queue wait time AND LLM processing
 * time, matching the user-facing SLA.
 *
 * Cancellation:
 *  - Per-entry: the entry's timeout fires → remove from queue OR abort the active LLM
 *    call if processing has already started.
 *  - Bulk: cancelAll() rejects every pending entry and aborts the active LLM call.
 *
 * The queue is processor-agnostic: the consumer registers a `processor` callback that
 * is invoked for each entry; the callback receives the AbortSignal so it can wire it
...
export class QuestionQueue {
	...
	public enqueue(
		...
		if (this._entries.length >= this._maxSize) {
			return Promise.reject(
				new Error(`Live Memory question queue is full (max ${this._maxSize}). Try again later.`),
			)
		}
		...
		return new Promise<QuestionResult>((resolve, reject) => {
			...
			const timeoutId = setTimeout(() => {
				...
				if (idx !== -1) {
					...
					reject(new Error(`Live Memory question timed out after ${timeoutMs}ms (in queue)`))
					return
				}
	...
	public cancelAll(): void {
		...
		for (const entry of pending) {
			entry.reject(new Error("Live Memory questions cancelled"))
		}
	}
	...
	private async _drain(): Promise<void> {
		...
		while (this._entries.length > 0) {
			...
			if (Date.now() - entry.startTime > entry.timeoutMs) {
				entry.reject(new Error(`Live Memory question timed out (queue wait for ${entry.timeoutMs}ms)`))
				continue
			}
