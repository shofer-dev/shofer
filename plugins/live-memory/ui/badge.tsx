...
const P: Record<string, string[]> = {
	...
	turns: [
		"M14 9a2 2 0 0 1-2 2H6l-4 4V4a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v5z",
	...
	eraser: [
		"M12 8c4.97 0 9-1.34 9-3s-4.03-3-9-3-9 1.34-9 3 4.03 3 9 3z",
		"M3 5v6c0 1.66 4.03 3 9 3",
		"M3 12v6c0 1.66 4.03 3 9 3",
		"m16 16 5 5",
		"m21 16-5 5",
	],
}
		...
		<span ref={wrapRef} style={{ position: "relative", display: "inline-flex" }}>
			...
			{open && (
				<div
					...
					{/* Enabled but unapproved: say so plainly and lead with the one action
					    that changes it, instead of info rows about an agent that cannot run. */}
					{s.needsConsent ? (
						<>
							<div style={{ padding: "10px 12px", display: "flex", flexDirection: "column", gap: 8 }}>
								...
								<div style={{ color: V.muted, lineHeight: 1.45 }}>
									Live Memory is enabled but not allowed to make <strong>billed AI calls</strong> yet,
									so it is not observing your workspace or answering questions. Approving it starts it
									immediately.
								</div>
							</div>
							<div style={{ padding: 8, borderTop: `1px solid ${V.border}`, display: "grid", gap: 6 }}>
								<ActionButton icon="settings" label="Approve billed AI calls…" onClick={approve} />
							</div>
						</>
					) : (
						<>
							<div style={{ padding: "8px 12px", display: "flex", flexDirection: "column", gap: 6 }}>
								...
								<InfoRow
									...
									value={String(s.contextFiles?.length ?? 0)}
								/>
								<InfoRow
									icon="turns"
									...
									sub={
										s.stats?.pendingQuestions
											? `${s.stats.pendingQuestions} question(s) queued`
											: undefined
									}
								/>
								{s.costSnapshot ? (
									<InfoRow
										...
										sub={
											s.costSnapshot.sessionInputTokens !== undefined &&
											s.costSnapshot.sessionOutputTokens !== undefined
