...
describe("QuestionQueue (plugin port)", () => {
	...
	it("forwards soft limits + context files to the processor", async () => {
		const q = new QuestionQueue()
		let seenFiles: string[] | undefined
		let seenLimits: { softTimeoutSec?: number; softResultLength?: number } | undefined
		q.setProcessor(async (_question, contextFiles, _signal, softLimits) => {
			seenFiles = contextFiles
			seenLimits = softLimits
			return fakeResult("ok")
		})
		await q.enqueue("q", ["a.ts", "b.ts"], undefined, { softTimeoutSec: 5, softResultLength: 100 })
		expect(seenFiles).toEqual(["a.ts", "b.ts"])
		expect(seenLimits).toEqual({ softTimeoutSec: 5, softResultLength: 100 })
	})
