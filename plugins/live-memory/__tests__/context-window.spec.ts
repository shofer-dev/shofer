...
describe("ContextWindow (plugin port)", () => {
	...
	describe("removeFileContext", () => {
		it("drops the named entry", () => {
			const cw = new ContextWindow()
			cw.upsertFileContext(makeFile("a.ts", 10))
			cw.upsertFileContext(makeFile("b.ts", 10))
			cw.removeFileContext("a.ts")
			expect(cw.fileContextPaths).toEqual(["b.ts"])
		})
	})
	...
	describe("enforceLimit", () => {
		...
		it("evicts file contexts before message pairs", () => {
			const cw = new ContextWindow({ maxContextTokens: 20 })
			cw.appendMessage(makeMessage("user", "hi")) // ~1 token
			cw.appendMessage(makeMessage("assistant", "there")) // ~2 tokens
			cw.upsertFileContext(makeFile("a.ts", 40, 100))
			cw.enforceLimit()
			// The file context goes first; the message pair survives.
