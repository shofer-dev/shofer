...
import plugin, { FILE_MUTATING_TOOLS, resolveDiffRange } from "../src/main.js"
import { CheckpointServiceRegistry } from "../src/service-registry.js"
	...
	const ctx = {
		...
		task: {
			marker: async (input: { kind: string; text: string; restorable?: boolean; suppress?: boolean }) => {
				markers.push({ ...input, ts: (ts += 10), pluginName: "checkpoints" })
			},
...
describe("checkpoints plugin — hooks", () => {
	...
	it("renders a diff through the host editor for a local:show-diff request", async () => {
		await plugin.handleRequest!("local:show-diff", { title: "T", changes: [] }, harness.ctx as never)
	...
	it("removes the task's shadow repository when the task is deleted", async () => {
		await plugin.lifecycle!.beforeToolCall!("write_to_file", {}, harness.ctx as never)
		const repoDir = path.join(storage, "tasks", "task-1")
...
describe("checkpoints plugin — diff range resolution", () => {
