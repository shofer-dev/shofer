...
// ../../node_modules/.pnpm/simple-git@3.27.0/node_modules/simple-git/dist/cjs/index.js
var require_cjs = __commonJS({
  "../../node_modules/.pnpm/simple-git@3.27.0/node_modules/simple-git/dist/cjs/index.js"(exports, module) {
    "use strict";
    ...
    var init_plugin_store = __esm({
      "src/lib/plugins/plugin-store.ts"() {
        "use strict";
        import_node_events = __require("node:events");
        init_utils();
        PluginStore = class {
          constructor() {
          ...
          append(type, action) {
            const plugin2 = append(this.plugins, { type, action });
            return () => this.plugins.delete(plugin2);
          }
          add(plugin2) {
            const plugins = [];
            asArray(plugin2).forEach((plugin22) => plugin22 && this.plugins.add(append(plugins, plugin22)));
            return () => {
              plugins.forEach((plugin22) => this.plugins.delete(plugin22));
            };
          }
    ...
    var simpleGit2 = esModuleFactory2(gitExportFactory2(gitInstanceFactory2));
    module.exports = Object.assign(simpleGit2, { gitP: gitP2, simpleGit: simpleGit2 });
  }
});
...
// src/shadow-git.ts
var import_simple_git = __toESM(require_cjs());
...
var getConfigFilePatterns = () => ["*.env*", "*.local", "*.development", "*.production"];
var getLargeDataFilePatterns = () => [
  "*.zip",
  "*.tar",
  "*.gz",
  "*.rar",
...
var getLfsPatterns = async (workspacePath) => {
  try {
    const attributesPath = join(workspacePath, ".gitattributes");
    if (await fileExistsAtPath(attributesPath)) {
      return (await fs2.readFile(attributesPath, "utf8")).split("\n").filter((line) => line.includes("filter=lfs")).map((line) => line.split(" ")[0].trim());
    }
  } catch {
  }
  return [];
};
var getExcludePatterns = async (workspacePath) => [
  ".git/",
...
function createSanitizedGit(baseDir, log) {
  const dotGitPath = path.join(baseDir, ".git");
  const sanitizedEnv = {};
  const removedVars = [];
  for (const [key, value] of Object.entries(process.env)) {
    if (key === "GIT_DIR" || key === "GIT_WORK_TREE" || key === "GIT_INDEX_FILE" || key === "GIT_OBJECT_DIRECTORY" || key === "GIT_ALTERNATE_OBJECT_DIRECTORIES" || key === "GIT_CEILING_DIRECTORIES" || key === "GIT_TEMPLATE_DIR") {
  ...
  const options = { baseDir, config: [] };
  const git = (0, import_simple_git.simpleGit)(options);
  git.env(sanitizedEnv);
  return git;
}
var ShadowGitRepo = class extends EventEmitter {
  taskId;
  checkpointsDir;
  workspaceDir;
  scopedWorktreeDir;
  ...
  async init() {
    ...
    await fs3.mkdir(this.checkpointsDir, { recursive: true });
  ...
  async getShadowGitConfigWorktree(git) {
    if (!this.shadowGitConfigWorktree) {
      try {
        this.shadowGitConfigWorktree = (await git.getConfig("core.worktree")).value || void 0;
      } catch (error) {
        this.log(
  ...
  async saveCheckpoint(message, options) {
    try {
      ...
      const commitArgs = options?.allowEmpty ? { "--allow-empty": null } : void 0;
  ...
  async getDiff({ from, to }) {
    ...
    for (const file of files) {
      ...
      const after = to ? await this.git.show([`${to}:${relPath}`]).catch(() => "") : await fs3.readFile(absPath, "utf8").catch(() => "");
  ...
  async getDiffStat({ from, to }) {
    ...
    return files.map((file) => {
      ...
      return {
        ...
        insertions: binary ? 0 : file.insertions ?? 0,
        deletions: binary ? 0 : file.deletions ?? 0,
        binary
      };
    });
  }
...
function arePathsEqual(path1, path22) {
  if (!path1 && !path22) return true;
  if (!path1 || !path22) return false;
  const normalize2 = (p) => {
  ...
  const a = normalize2(path1);
  const b = normalize2(path22);
  return process.platform === "win32" ? a.toLowerCase() === b.toLowerCase() : a === b;
}

// src/service-registry.ts
var CheckpointServiceRegistry = class {
  constructor(options) {
  ...
  repoDir(taskId) {
    return path2.join(this.options.storageDir, "tasks", taskId);
  }
   ...
   */
  async get(request) {
    if (this.disabled.has(request.taskId)) return void 0;
  ...
  async create(request) {
    const scopedWorktreeDir = request.cwd && request.cwd !== request.workspaceDir ? request.cwd : void 0;
...
var state = { lastSnapshotTurn: /* @__PURE__ */ new Map() };
function config(ctx) {
  const c = ctx.config ?? {};
  const initTimeoutSeconds = typeof c.initTimeoutSeconds === "number" && c.initTimeoutSeconds > 0 ? c.initTimeoutSeconds : 15;
  const raw = typeof c.excludePatterns === "string" ? c.excludePatterns : "";
  return {
    initTimeoutMs: initTimeoutSeconds * 1e3,
    extraExcludePatterns: raw.split("\n").map((line) => line.trim()).filter(Boolean)
  };
}
function registry() {
  const ctx = state.ctx;
  ...
  state.registry = new CheckpointServiceRegistry({
    ...
    onCheckpoint: (taskId, event) => {
      ...
      }).catch((error) => ctx.host?.log.warn(`failed to append checkpoint marker: ${String(error)}`));
    }
  });
...
async function snapshot(ctx, taskId, options = {}) {
  const reg = registry();
  if (!reg) return;
...
function resolveDiffRange(hashes, commitHash, mode) {
  ...
  switch (mode) {
    case "checkpoint":
      return {
        from: commitHash,
        to: index !== -1 && index < hashes.length - 1 ? hashes[index + 1] : void 0,
    ...
    case "to-current":
      return { from: commitHash, to: void 0, title: "Changes from this checkpoint to now" };
    case "full":
      return { from: hashes[0], to: void 0, title: "Changes since the first checkpoint" };
    default:
      return { notice: "no-previous" };
  }
}
async function computeDiff(ctx, request) {
...
async function restore(ctx, request) {
  const taskId = ctx.taskId;
  const reg = registry();
  const workspaceDir = workspaceFor(ctx);
...
var plugin = {
  ...
  lifecycle: {
     ...
     */
    async beforeToolCall(toolName, _args, ctx) {
      const taskId = ctx.taskId;
     ...
     */
    async onTimelineRewind(info, ctx) {
    ...
    async onTaskDeleted(info, ctx) {
      const reg = registry();
      if (!reg) return;
      try {
        await fs4.rm(reg.repoDir(info.taskId), { recursive: true, force: true });
      } catch (error) {
  ...
  async handleRequest(method, params, ctx) {
    switch (method) {
      case "diff":
        return computeDiff(ctx, params);
      case "restore":
        return restore(ctx, params);
      case "list":
        return markers(ctx.taskId);
      case "local:show-diff": {
        const { title, changes } = params;
