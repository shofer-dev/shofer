...
```mermaid
flowchart TD
    B["plugin .slang — plugins/builtin-config/workflows/<br/>contributed by an enabled plugin, lowest priority"]
    ...
    B -.->|"layer skipped when<br/>SHOFER_DISABLED_PLUGINS names builtin-config"| D
    G --> D
    P --> D
    D --> M
    M --> L
...
the available workflow set. The control is naming the plugin in
**`SHOFER_DISABLED_PLUGINS`** (`SHOFER_DISABLED_PLUGINS=builtin-config`,
comma-separated, truthy entries only by presence), delivered on the executor /
...
`globalSettingsSchema` or the Settings UI and cannot be toggled from the webview. Note
the granularity: disabling `builtin-config` removes the built-in modes along with the
workflows — a deployment that replaces the defaults replaces the whole set.
...
(re-exported from `@shofer/core`) reads the list into what `PluginManager` consumes as
`forceDisabledPlugins`, ignoring the user's enable state. A suppressed plugin
...
| [`packages/core/src/config/governance.ts`](../../../packages/core/src/config/governance.ts)                           | `governanceDisabledPlugins()` — the `SHOFER_DISABLED_PLUGINS` reader                                                  |
...
| [`src/extension/api.ts`](../../../src/extension/api.ts)                                                               | Public API — `ShoferExtensionApi.discoverWorkflows()`, `ShoferExtensionApi.createWorkflow()`                          |
...
- [`modes.md`](modes.md) — the built-in modes, which give the
