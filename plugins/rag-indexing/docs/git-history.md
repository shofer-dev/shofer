...
| File                                                         | Purpose                                                                  |
| ------------------------------------------------------------ | ------------------------------------------------------------------------ | -------- | ------- | ----- |
| `../src/git-index-service/interfaces/git.ts`                 | `GitCommitBlock`, `IGitLogExtractor`, `IGitWatcher`, `IGitSearchService` |
| `../src/git-index-service/processors/git-log-extractor.ts`   | `GitLogExtractor`: runs `git log`, parses output                         |
| `../src/git-index-service/processors/git-watcher.ts`         | Stub (no-op in Phase 1, implemented in Phase 2)                          |
| `../src/git-index-service/git-cache-manager.ts`              | `GitCacheManager`: per-commit SHA-256 hash cache                         |
| `../src/git-index-service/git-state-manager.ts`              | `GitHistoryStateManager`: Standby                                        | Indexing | Indexed | Error |
| `../src/git-index-service/git-search-service.ts`             | `GitSearchService`: embed query → Qdrant search                          |
| `../src/git-index-service/git-index-manager.ts`              | `GitIndexManager`: singleton, orchestrates lifecycle                     |
...
3. **Truncation limit was wrong** — the error-handling table claimed commit messages are truncated at 8000 characters; the actual constant `MAX_CONTENT_LENGTH` in [`git-log-extractor.ts`](../src/git-index-service/processors/git-log-extractor.ts:38) is `4000`. Corrected.
...
6. **Indexing pipeline description misses the catch-up step** — the prose says "filter by maxHistoryDays" after extraction, but the actual `startIndexing()` in [`git-history-orchestrator.ts`](../src/git-index-service/git-history-orchestrator.ts:88) performs a catch-up incremental scan first (when `lastCommitDate` exists in cache), then the full scan. The data-flow diagram does not show this catch-up path.
