...
import { LanguageParser, loadRequiredLanguageParsers } from "../../core-shared.js"
import { parseMarkdown } from "../../core-shared.js"
import { incCodeIndexError } from "../../plugin-runtime.js"
import { grammarDirectory } from "../../grammars.js"
...
export class CodeParser implements ICodeParser {
	...
	async parseFile(
		...
		if (options?.content) {
			...
			try {
				...
				codeIndexLog.error(`Error reading file ${filePath}:`, error)
				incCodeIndexError("parseFile")
				return []
			}
		}
	...
	private async parseContent(filePath: string, content: string, fileHash: string): Promise<CodeBlock[]> {
		...
		if (!this.loadedParsers[ext]) {
			...
			if (pendingLoad) {
				try {
					...
					codeIndexLog.error(`Error in pending parser load for ${filePath}:`, error)
					incCodeIndexError("parseContent:loadParser")
					return []
				}
			} else {
				const loadPromise = loadRequiredLanguageParsers([filePath], grammarDirectory())
				...
				try {
					...
					codeIndexLog.error(`Error loading language parser for ${filePath}:`, error)
					incCodeIndexError("parseContent:loadParser")
					return []
				} finally {
		...
		if (!language) {
			codeIndexLog.warn(`No parser available for file extension: ${ext}`)
			return []
		}
