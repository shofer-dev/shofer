...
import { EmbedderInfo, EmbeddingResponse, IEmbedder } from "../interfaces/index.js"
...
export class CodeIndexOllamaEmbedder implements IEmbedder {
	...
	async createEmbeddings(texts: string[], model?: string, signal?: AbortSignal): Promise<EmbeddingResponse> {
		...
		try {
			...
			incCodeIndexError("OllamaEmbedder:createEmbeddings")
			...
			"ollama",
			{
				...
				beforeStandardHandling: (error: any) => {
					...
					if (
						...
						incCodeIndexError("OllamaEmbedder:validateConfiguration:connectionFailed")
						return {
							valid: false,
							error: t("embeddings:ollama.serviceNotRunning", { baseUrl: this.baseUrl }),
						}
					} else if (error?.code === "ENOTFOUND" || error?.message?.includes("ENOTFOUND")) {
						incCodeIndexError("OllamaEmbedder:validateConfiguration:hostNotFound")
						return {
							valid: false,
							error: t("embeddings:ollama.hostNotFound", { baseUrl: this.baseUrl }),
						}
					} else if (error?.name === "AbortError") {
						incCodeIndexError("OllamaEmbedder:validateConfiguration:timeout")
						// Handle timeout
						return {
							valid: false,
