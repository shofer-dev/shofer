...
import { MAX_ITEM_TOKENS, INITIAL_RETRY_DELAY_MS } from "../../constants/index.js"
import { codeIndexLog } from "../../../logging.js"

// Mock the i18n `t` used directly by the embedder. The error handler (handleOpenAIError)
// resolves its message via core's own i18n instance, which returns the raw key under
// NODE_ENV=test — so we leave the default i18next instance untouched.
vitest.mock("../../../i18n.js", async (importOriginal) => ({
...
vitest.mock("../../../logging.js", async (importOriginal) => ({
	...((await importOriginal()) as Record<string, unknown>),
	codeIndexLog: {
		error: vitest.fn(),
		info: vitest.fn(),
...
describe("OpenAICompatibleEmbedder", () => {
	...
	describe("constructor", () => {
		...
		it("should handle API key with invalid characters (ByteString conversion error)", () => {
			...
			// The plugin normalises SDK errors itself (`engine/openai-error.ts`), so the
			// assertion is on the message a user actually reads — the invalid-characters
