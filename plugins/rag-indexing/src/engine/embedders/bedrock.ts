...
import { getDefaultModelId } from "../../core-shared.js"
import { hostVersion } from "../../plugin-runtime.js"
import { t } from "../../i18n.js"
import { withValidationErrorHandling, formatEmbeddingError } from "../shared/validation-helpers.js"
...
export class BedrockEmbedder implements IEmbedder {
	...
	constructor(
		...
		this.bedrockClient = new BedrockRuntimeClient({
			userAgentAppId: `Shofer#${hostVersion()}`,
			region: this.region,
			credentials,
		})
	...
	private async _embedBatchWithRetries(
		...
		for (let attempts = 0; attempts < MAX_RETRIES; attempts++) {
			try {
				...
				incCodeIndexError("BedrockEmbedder:_embedBatchWithRetries")
				// Log the error for debugging
				codeIndexLog.error(`Bedrock embedder error (attempt ${attempts + 1}/${MAX_RETRIES}):`, error)
	...
	async validateConfiguration(): Promise<{ valid: boolean; error?: string }> {
		return withValidationErrorHandling(async () => {
			try {
				...
				incCodeIndexError("BedrockEmbedder:validateConfiguration")
				throw error
			}
		}, "bedrock")
	}
