...
import { getDefaultModelId, getModelQueryPrefix } from "../../core-shared.js"
	...
	private async _embedBatchWithRetries(
		...
		for (let attempts = 0; attempts < MAX_RETRIES; attempts++) {
			...
			try {
				...
				incCodeIndexError("OpenRouterEmbedder:_embedBatchWithRetries")
				...
				codeIndexLog.error(`OpenRouter embedder error (attempt ${attempts + 1}/${MAX_RETRIES}):`, error)
	...
	async validateConfiguration(): Promise<{ valid: boolean; error?: string }> {
		return withValidationErrorHandling(async () => {
			try {
				...
				incCodeIndexError("OpenRouterEmbedder:validateConfiguration")
				throw error
			}
		}, "openrouter")
	}
