...
export class OpenAiEmbedder implements IEmbedder {
	...
	/**
	 * The handler options, held directly.
	 *
	 * This used to extend core's `OpenAiNativeHandler` for exactly this field — which
	 * dragged core's whole API layer (and its tiktoken WASM) into the plugin bundle, where
	 * the `.wasm` does not exist and the plugin therefore failed to load at all.
	 */
	...
	private async _embedBatchWithRetries(
		...
		for (let attempts = 0; attempts < MAX_RETRIES; attempts++) {
			try {
				...
				incCodeIndexError("OpenAiEmbedder:_embedBatchWithRetries")
	...
	async validateConfiguration(): Promise<{ valid: boolean; error?: string }> {
		return withValidationErrorHandling(async () => {
			try {
				...
				incCodeIndexError("OpenAiEmbedder:validateConfiguration")
				throw error
			}
		}, "openai")
	}
