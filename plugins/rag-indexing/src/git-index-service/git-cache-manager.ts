...
export class GitCacheManager {
	 ...
	 * popover.
	 */
	private readonly _onCacheUpdated = new TypedEmitter<string>()
	public readonly onCacheUpdated = this._onCacheUpdated.event

	constructor(storageDir: string, workspacePath: string) {
		const wsHash = crypto.createHash("sha256").update(workspacePath).digest("hex").substring(0, 16)
		this._cachePath = path.join(storageDir, `git-index-cache-${wsHash}.json`)
	}
	...
	async initialize(): Promise<void> {
		try {
			const cacheData = await fs.readFile(this._cachePath, "utf8")
	...
	async persist(): Promise<void> {
		...
		await fs.writeFile(this._cachePath, data)
	}
