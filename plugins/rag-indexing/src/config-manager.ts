import { ApiHandlerOptions } from "@shofer/types"
import { EmbedderProvider } from "@shofer/types"
import { CodeIndexConfig, PreviousConfigSnapshot } from "./engine/interfaces/index.js"
import { DEFAULT_SEARCH_MIN_SCORE, DEFAULT_MAX_SEARCH_RESULTS } from "./engine/constants/index.js"
import { getDefaultModelId, getModelDimension, getModelScoreThreshold } from "./core-shared.js"
import { codeIndexLog } from "./logging.js"
import { setting } from "./plugin-runtime.js"
/**
...
export class CodeIndexConfigManager {
	...
	private _loadAndSetConfiguration(): void {
		...
		const codebaseIndexQdrantUrl = setting("qdrantUrl", "http://localhost:6333")
		const codebaseIndexEmbedderProvider = setting<string>("embedderProvider", "openai")
		const codebaseIndexEmbedderBaseUrl = setting("embedderBaseUrl", "")
		const codebaseIndexEmbedderModelId = setting("embedderModelId", "")
		const codebaseIndexSearchMinScore = setting<number | undefined>("searchMinScore", undefined)
		const codebaseIndexSearchMaxResults = setting<number | undefined>("searchMaxResults", undefined)

		const openAiKey = setting("openAiApiKey", "")
		const qdrantApiKey = setting("qdrantApiKey", "")
		const openAiCompatibleBaseUrl = setting("openAiCompatibleBaseUrl", "")
		const openAiCompatibleApiKey = setting("openAiCompatibleApiKey", "")
		const geminiApiKey = setting("geminiApiKey", "")
		const mistralApiKey = setting("mistralApiKey", "")
		const vercelAiGatewayApiKey = setting("vercelAiGatewayApiKey", "")
		const bedrockRegion = setting("bedrockRegion", "us-east-1")
		const bedrockProfile = setting("bedrockProfile", "")
		const openRouterApiKey = setting("openRouterApiKey", "")
		const openRouterSpecificProvider = setting("openRouterSpecificProvider", "")
		...
		const rawDimension = setting<unknown>("embedderModelDimension", undefined)
		if (rawDimension !== undefined && rawDimension !== null) {
			const dimension = Number(rawDimension)
			if (!isNaN(dimension) && dimension > 0) {
				this.modelDimension = dimension
			} else {
				codeIndexLog.warn(`Invalid embedderModelDimension value: ${rawDimension}. Must be a positive number.`)
	...
	public async loadConfiguration(): Promise<{
		...
		// The host re-resolves `ctx.config` (settings + secrets) on every plugin reload, so
		// re-reading it is all a refresh needs to be.
		this._loadAndSetConfiguration()
