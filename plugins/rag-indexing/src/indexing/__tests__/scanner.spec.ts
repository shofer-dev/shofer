...
vi.mock("fs/promises", () => {
	// The scanner reads file contents through `fs.readFile` (it used the editor's
	// filesystem API before the move), so the default export must carry it.
	const readFile = vi.fn().mockResolvedValue("test content")
	return {
		default: { readFile, writeFile: vi.fn(), mkdir: vi.fn(), access: vi.fn(), rename: vi.fn(), constants: {} },
		readFile,
		stat: vi.fn(),
	}
})
...
// The helpers the scanner borrows from core (`core-shared.ts`): the file walk is driven
// by the test, and the ignore controller is stubbed to allow everything.
...
describe("DirectoryScanner", () => {
	...
	beforeEach(async () => {
		...
		const { listFiles } = await import("../../core-shared.js")
