...
async function createWorktree(ctx: PluginContext, request: CreateRequest): Promise<WorktreeResult> {
	...
	try {
		...
		ctx.host?.log.warn(`[${PLUGIN_NAME}] could not update .gitignore: ${String(error)}`)
	}
	...
	if (result.success && result.worktree && options.copyWorktreeInclude !== false) {
		try {
			...
			ctx.host?.log.warn(`[${PLUGIN_NAME}] worktreeinclude copy failed: ${String(error)}`)
			step("copy", "failed")
		}
	}
	...
	if (result.success && result.worktree && options.initSubmodules !== false) {
		...
		if (!submodules.success) {
			...
			ctx.host?.log.warn(`[${PLUGIN_NAME}] submodule init failed; removing ${result.worktree.path}`)
			const cleanup = await worktreeService.deleteWorktree(cwd, result.worktree.path, true)
			if (!cleanup.success) {
				ctx.host?.log.warn(`[${PLUGIN_NAME}] cleanup after failed submodule init: ${cleanup.message}`)
			}
...
const plugin: ShoferPlugin = {
	...
	initialize(ctx: PluginContext): void {
		state.ctx = ctx
		state.pendingCwd = undefined
		ctx.host?.log.info("worktrees plugin initialized")
	},

	/**
	 * The UI's request surface, and core's one question.
	 *
