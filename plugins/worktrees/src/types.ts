 ...
 *
 * Plugin-local rather than in `@shofer/types`: worktrees are this plugin's data model,
 * shared only between its extension half and its two UI bundles. Core has no opinion
 * about them, which is what lets the plugin be removed without leaving a type behind.
 */
...
export interface WorktreeResult {
	...
	success: boolean
	/** Message code or text describing the result (the UI translates known codes) */
	message: string
	/** The worktree that was affected (if applicable) */
	worktree?: Worktree
	/** How many `.shofer/worktreeinclude` entries were copied into a new worktree. */
	copiedItems?: number
}
