...
function ask(api, method, params, mutates = false) {
  return api.request(`local:worktrees:${method}`, params, { mutates });
}
function useWorktreesFeature(api) {
  const [featureOn, setFeatureOn] = useState(true);
  useEffect(() => {
    let cancelled = false;
    api.request("local:features").then((features) => {
      if (cancelled) return;
      setFeatureOn(features?.worktrees !== false);
    }).catch(() => {
    });
    return () => {
      cancelled = true;
