...
function FileChangesPanel({ api }) {
  ...
  const featureOnPromise = useRef(null);
  const featureEnabled = useCallback(() => {
    featureOnPromise.current ?? (featureOnPromise.current = api.request("local:features").then((features) => features?.["file-changes"] !== false).catch(() => true).then((on) => {
      setFeatureOn(on);
      return on;
    }));
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const result = await api.request("file-changes:get");
      setPayload(result);
    } catch (err) {
  ...
  const showDiff = useCallback(
    async (entry) => {
      ...
      try {
        const diff = await api.request("file-changes:diff", { path: entry.path });
        if (!diff) return;
