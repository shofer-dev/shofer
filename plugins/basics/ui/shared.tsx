...
/** Call a worktrees-feature method on the host that owns the workspace. */
export function ask<T>(api: PluginUIApi, method: string, params?: unknown, mutates = false): Promise<T> {
	return api.request(`local:worktrees:${method}`, params, { mutates }) as Promise<T>
}

/**
 * Whether the worktrees feature is on in the Basics plugin's config/governance —
 * both worktree UI bundles render nothing when it is off (a deployment replacing the
 * feature must not end up with two branch chips). Optimistic `true` until answered,
 * so the chip does not flicker in the common enabled case.
 */
export function useWorktreesFeature(api: PluginUIApi): boolean {
	const [featureOn, setFeatureOn] = useState(true)
	useEffect(() => {
		let cancelled = false
		api.request("local:features")
			.then((features) => {
				if (cancelled) return
				setFeatureOn((features as Record<string, boolean> | undefined)?.worktrees !== false)
			})
			.catch(() => {})
		return () => {
			cancelled = true
