...
export default function FileChangesPanel({ api }: { api: PluginUIApi }) {
	...
	// Whether the file-changes feature is on in the Basics plugin's config/governance.
	// Resolved once per mount (before the first read, so a disabled feature never
	// flashes an error) and cached; off ⇒ this panel renders nothing.
	const [featureOn, setFeatureOn] = useState(true)
	const featureOnPromise = useRef<Promise<boolean> | null>(null)
	const featureEnabled = useCallback(() => {
		featureOnPromise.current ??= api
			.request("local:features")
			.then((features) => (features as Record<string, boolean> | undefined)?.["file-changes"] !== false)
			.catch(() => true)
			.then((on) => {
				setFeatureOn(on)
				return on
			})
		return featureOnPromise.current
	}, [api])
	...
	const refresh = useCallback(async () => {
		...
		try {
			const result = (await api.request("file-changes:get")) as ChangedFilesPayload | undefined
			setPayload(result)
		} catch (err) {
			flash(err instanceof Error ? err.message : String(err))
		}
	}, [api, flash, featureEnabled])
	...
	const act = useCallback(
		async (method: string, params?: unknown) => {
			try {
				await api.request(`file-changes:${method}`, params, { mutates: true })
			} catch (err) {
				flash(err instanceof Error ? err.message : String(err))
			}
	...
	const showDiff = useCallback(
		async (entry: ChangedFileEntry) => {
			...
			try {
				...
				const diff = (await api.request("file-changes:diff", { path: entry.path })) as
					| { title: string; changes: unknown[] }
					| undefined
				if (!diff) return
				// … then opened HERE, because the editor is this host's.
				await api.request("local:file-changes:show-diff", diff)
			} catch (err) {
				flash(err instanceof Error ? err.message : String(err))
			}
		},
