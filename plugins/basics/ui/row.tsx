	...
	const showDiff = useCallback(
		async (mode: DiffMode) => {
			...
			try {
				...
				const result = (await api.request("checkpoints:diff", { commitHash: marker.text, mode })) as DiffResult
				if (result?.notice) {
					flash(t(`notice.${result.notice}`))
					return
				}
				// … then rendered HERE, because the viewer is this host's, not the executor's.
				await api.request("local:checkpoints:show-diff", { title: result.title, changes: result.changes })
			} catch (error) {
