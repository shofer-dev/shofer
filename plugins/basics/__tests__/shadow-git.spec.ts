// Ported from the built-in checkpoints suite when checkpoints became a plugin.
...
import { fileExistsAtPath } from "../src/checkpoints/fs-util.js"
import { ShadowGitRepo } from "../src/checkpoints/shadow-git.js"

const tmpDir = path.join(os.tmpdir(), "CheckpointsPlugin")

/**
 * Build a repo the way the plugin's registry does: one shadow repo per task, under
...
const createRepo = ({
	...
	new ShadowGitRepo({
		taskId,
		workspaceDir,
		checkpointsDir: path.join(shadowDir, "tasks", taskId),
		scopedWorktreeDir,
		log: () => {},
	})
...
describe("ShadowGitRepo", () => {
	{
		// Retained from the ported suite: names the temp dirs per describe block.
		const prefix = "ShadowGitRepo"
		const taskId = "test-task"

		let workspaceGit: SimpleGit
		let testFile: string
		let service: ShadowGitRepo
		...
		beforeEach(async () => {
			...
			service = createRepo({ taskId, shadowDir, workspaceDir })
			await service.init()
		})
		...
		describe("ShadowGitRepo#getDiff", () => {
		...
		describe("ShadowGitRepo#saveCheckpoint", () => {
			...
			it("does not create a checkpoint for LFS files", async () => {
				...
				service = new ShadowGitRepo({
					taskId: service.taskId,
					workspaceDir: service.workspaceDir,
					checkpointsDir: service.checkpointsDir,
					log: () => {},
				})
		...
		describe("ShadowGitRepo#init", () => {
			it("initializes a git repository if one does not already exist", async () => {
				...
				const newService = createRepo({ taskId, shadowDir, workspaceDir })
				const { created } = await newService.init()
		...
		describe("ShadowGitRepo with nested git repositories", () => {
			it("succeeds when nested git repositories are detected (GIT_DIR prevents submodule detection)", async () => {
				...
				const service = createRepo({ taskId, shadowDir, workspaceDir })
		...
		describe("ShadowGitRepo#saveCheckpoint with allowEmpty option", () => {
...
describe("worktree path comparison", () => {
	it("accepts core.worktree with trailing newline from git output", async () => {
		...
		try {
			...
			const service1 = new ShadowGitRepo({
				taskId: "trim-test",
				checkpointsDir: shadowDir,
				workspaceDir,
				log: () => {},
			})
			await service1.init()
			...
			const service2 = new ShadowGitRepo({
				taskId: "trim-test-2",
				checkpointsDir: shadowDir,
				workspaceDir,
				log: () => {},
	...
	it("throws when core.worktree is missing", async () => {
		...
		try {
			...
			const service2 = new ShadowGitRepo({
				taskId: "missing-test-2",
				checkpointsDir: shadowDir,
				workspaceDir,
				log: () => {},
			})
			await expect(service2.init()).rejects.toThrowError(/core\.worktree to be set/)
		} finally {
			vitest.restoreAllMocks()
			await fs.rm(shadowDir, { recursive: true, force: true })
			await fs.rm(workspaceDir, { recursive: true, force: true })
	...
	it("scopes core.worktree to scopedWorktreeDir when provided", async () => {
		...
		try {
			...
			const service = new ShadowGitRepo({
				taskId: "scoped-test",
				checkpointsDir: shadowDir,
				workspaceDir,
				scopedWorktreeDir: scopedDir,
				log: () => {},
			})
			await service.init()
