...
// `ignore` is vendored (see `build-ui.mjs`): the loader resolves bare imports only
// through the shipped plugin SDK, so a bare `import "ignore"` would fail in an
// installed extension. Types still come from the real package (erased at compile time).
import ignore from "../vendor/ignore.mjs"
import type { Ignore } from "ignore"

import type { WorktreeIncludeStatus } from "./types.js"

/** How often the copy watcher samples byte progress while a copy is running. */
const WORKTREE_POLL_INTERVAL_MS = 500
...
export class WorktreeIncludeService {
	...
	private async copyDirectoryWithProgress(
		...
		const pollInterval = WORKTREE_POLL_INTERVAL_MS
		let polling = true
