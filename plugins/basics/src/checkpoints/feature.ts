/**
 * Checkpoints — per-task undo history, as a feature of the Basics plugin.
 *
 ...
 * | `enableCheckpoints` / `checkpointTimeout` settings | the feature toggle + manifest `config`     |
 *
 * Disabling the feature is flipping `checkpoints: false` in the Basics config (or the
 * `basics:checkpoints` governance entry); nothing in core knows it exists.
 */
...
function config(ctx: PluginContext) {
	...
	const initTimeoutSeconds =
		typeof c.checkpointInitTimeoutSeconds === "number" && c.checkpointInitTimeoutSeconds > 0
			? c.checkpointInitTimeoutSeconds
			: 15
	const raw = typeof c.checkpointExcludePatterns === "string" ? c.checkpointExcludePatterns : ""
	return {
...
function registry(): CheckpointServiceRegistry | undefined {
	...
	state.registry = new CheckpointServiceRegistry({
		// Scoped below the plugin's storage: the Basics features share ONE plugin
		// storage dir, and file-changes keeps per-task state under `tasks/` too.
		storageDir: path.join(ctx.storage.dir, "checkpoints"),
		initTimeoutMs,
...
export const checkpointsFeature: BasicsFeature = {
	id: "checkpoints",
	...
	initialize(ctx: PluginContext): void {
		...
		ctx.host?.log.info("checkpoints feature initialized")
	},
	...
	/**
	 * Request/response surface for this feature's own UI (and, for a task running on a
	 * remote executor, for the controller reaching that executor). Methods arrive bare:
	 * the UI sends `checkpoints:diff` / `local:checkpoints:show-diff` and `main.ts`
	 * strips the routing and namespace prefixes.
	 *
	 * `diff` and `restore` are answered where the task's shadow repo lives, so they
	 * are routed to the owning host. `show-diff` rides the `local:` routing prefix and
	 * is answered on the host with the user's editor — the executor has no viewer.
	 */
