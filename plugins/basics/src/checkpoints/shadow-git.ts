...
// `simple-git` is vendored (`build-ui.mjs` pre-bundles it into `src/vendor/`): the
// loader bundles this TypeScript at load time resolving bare imports through the
// shipped plugin SDK, which contains `@shofer/types` and nothing else — a bare
// `import "simple-git"` resolves in this repo and fails in an installed extension.
// Types still come from the real package (erased at compile time).
import { simpleGit } from "../vendor/simple-git.mjs"
