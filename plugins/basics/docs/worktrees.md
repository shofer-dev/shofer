...
commands and the decision of _where a task runs_ — lives in the bundled **Basics**
plugin's worktrees feature ([`plugins/basics/src/worktrees/`](../src/worktrees/); see
[`../DESIGN.md`](../DESIGN.md) for the composition and feature switches). **The core side** — the execution model (a task has a
...
flowchart TD
    ...
    subgraph plug["plugins/basics — worktrees feature"]
        MAIN["src/worktrees/feature.ts — request surface"]
...
Both bundles talk to `src/worktrees/feature.ts` over the plugin UI channel
(`api.request`), which
replaces eleven webview IPC message types and the eight extension→webview replies they
were paired with.

Every request is prefixed **`local:worktrees:`** — `local:` because without it a
request made while a remote executor's task is focused would be answered by that
executor (listing _its_ checkout for a panel that manages the repository open here;
worktrees are directories on this machine, and the prefix pins them to the host with
the workspace), and `worktrees:` because the Basics features share one request surface
and their method names would otherwise collide (see
...
`/basics:merge-worktree`, at the built-in precedence tier — so a project's own
...
The checkpoints feature of this same plugin ([`checkpoints.md`](checkpoints.md))
scopes its shadow git to `task.cwd`,
...
snapshots. Neither feature knows about the other: both key off the task's directory,
which is core's.
...
- [`checkpoints.md`](checkpoints.md) — per-task undo, scoped the
  same way
