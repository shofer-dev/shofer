...
<p align="center">
  <em>Hızlı yardım alın → <a href="https://discord.gg/shofer">Discord'a Katılın</a> • Asenkron mu tercih edersiniz? → <a href="https://www.reddit.com/r/Shofer/">r/Shofer'a Katılın</a></em>
</p>
...
- Poe, Shofer içinde Poe modellerine doğrudan erişebilmen için bir AI sağlayıcısı olarak eklendi.
...
## Shofer SİZİN İçin Ne Yapabilir?
...
Shofer, sizin çalışma şeklinize uyum sağlar, tam tersi değil:
...
Shofer uzantısını çalıştırmanın birkaç yolu vardır:

### Geliştirme Modu (F5)

Aktif geliştirme için VSCode'un yerleşik hata ayıklama özelliğini kullanın:

VSCode'da `F5` tuşuna basın (veya **Çalıştır** → **Hata Ayıklamayı Başlat**'a gidin). Bu, Shofer uzantısının çalıştığı yeni bir VSCode penceresi açacaktır.
...
1.  İlk olarak, VSIX paketini oluşturun:
    ...
    pnpm vsix
    ```
2.  `bin/` dizininde bir `.vsix` dosyası oluşturulur (ör. `bin/shofer-<version>.vsix`).
...
**Shofer'un keyfini çıkar!** Onu kısa tasmalı tut ya da otonom dolaşsın; ne inşa edeceğini görmek için sabırsızlanıyoruz. Soruların veya özellik fikirlerin varsa [Reddit topluluğumuza](https://www.reddit.com/r/Shofer/) ya da [Discord](https://discord.gg/shofer)'a uğra. İyi kodlamalar!
