...
- You MUST use `safeWriteJson(filePath: string, data: any): Promise<void>` imported from `@shofer/core` (the `packages/core` package) instead of `JSON.stringify` with file-write operations
