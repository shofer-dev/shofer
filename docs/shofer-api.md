...
| `sendMessage(taskId, message, images?, trace?)`                                           | Send a follow-up message to a task. Rehydrates it when it is finished or cold — queued before the resume starts, so no resume ask is raised and a completed conversation continues without a separate `resumeTask`. `trace` is the same W3C trace context `createTask` takes, and it matters MORE here: a conversation is created once and messaged for the rest of its life, so a controller that states it only at creation attributes exactly the first turn of every multi-turn run. It reaches observers as `UserMessageInfo.trace` (on the rehydrated turn too, where the queued message IS the resumption).                                                                                                                                                                                                |
...
#### A conversation's stream carries one task's events
...
its own, so its events go on its own stream; that is fine, because every child runs
CONCURRENTLY with its parent and routes its question to that parent's mailbox, which is
running and can answer ([parallelism.md](./parallelism.md)). A controller that wants a
child's transcript subscribes to the child, or reads the worker-wide `/api/v1/event`.

An answer goes back through `respondToAsk` addressed at the task, echoing the ask's
`askId`; `respondToAsk` follows that id to whichever live task of the conversation is
parked on it, so an answer addressed at the root still reaches a child that raised the
ask. Answers with no `askId`, or whose `askId` the addressed task itself is parked on,
...
POST /api/v1/task/:id/message     { message, images?, trace? } → sendMessage()
                                  (also honours W3C traceparent/tracestate request headers)
...
enabling programmatic consumption from the CLI, companion extensions, and
...
flowchart LR
    ...
    SURF["task lifecycle · history &amp; export · configuration<br/>provider profiles · output logs"]
