...
Two operations extend the vendored spec beyond `stake` / `await` / `commit` /
`escalate`:

| Operation                    | Status               | Description                                                                                                                                                                                                                                                                                                                                                        |
| ---------------------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `output: { … } where <expr>` | ✅ Parsed + enforced | The `where` clause is a **semantic** assertion over the parsed result, checked after the structural schema. The two layers fail differently and need different corrections: a structural miss means "answer in this shape", a semantic one means "the shape was right, change the values". No provider enforces a predicate, so this is always executor-side.      |
| `call fn(...) -> @dest`      | ✅ Parsed            | The deterministic twin of `stake`: a registered FUNCTION rather than an agent. It **blocks** exactly as a stake does — the VM hands it out and waits, because what runs and where it may run is the dispatcher's decision, not the interpreter's. An agent composes registered functions and cannot define one, which is what keeps a spec inert, reviewable data. |
...
| `requires: <tag expr>`                          | ⬜ Parsed, backend-dependent | N/A                            | Boolean capability expression over runner tags (`browser and (gpu or highmem)`; `[a, b]` is sugar for `and`; no negation). **Ignored by this native backend** — it has no runners, so there is nothing to select between. Under the Temporal backend it canonicalizes to DNF and hashes to a task-queue name, and runners self-select by polling. Stated here rather than omitted so the clause's meaning is unambiguous per backend rather than silently different. See [slang_specs.md](slang_specs.md#capability-routing--requires). |
