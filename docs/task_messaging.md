...
**Status:** the mailbox is what this document describes and what the code does —
every message that reaches a task, on all four planes, is an envelope in a box.
**Built:** the mailbox core and the three agent-facing tools (an envelope, a
per-task persisted box, the `environment_details` digest, the wake path,
`send_message` / `reply` / `wait`, auto-approval, chat rows and telemetry);
parent/child on the mailbox (`new_task` is concurrent-only, a child's result and
a child's question are envelopes); the plugin API's one delivery door; and the
mesh's mailbox transport. The peer-messaging path, the blocking `new_task`,
`wait_for_task`, `answer_subtask_question`, `ctx.agent.notify` and the mesh's own
messaging verbs are deleted, and the retired vocabulary is now caught by the
repo's docs-hygiene check. **Owed:** live verification on staging of each plane
in turn — in-process, parent/child, bus, A2A, Temporal — and, gated on it, the
`integration-tests/` coverage that a verified surface earns. The per-step register
...
- `wait` renders its own `ChatRow` case (the timeout and any wake condition);
  `reply` renders one row per item.
...
| **6** | sweep and close: the retired vocabulary recorded in the docs-hygiene check and every live-sounding mention of it rewritten across the corpus, downstream repos' own references noted on their own cadence, and the CHANGELOG entry — all **done**. Still owed: the `integration-tests/` coverage the new tools earn once each plane is verified live, and the deletion of the now-unreachable sync-child escalation machinery (`escalateFollowupToConversation`, `confirmNoConversationDriver`, `SUBSCRIBER_REATTACH_GRACE_MS` and the AGENTS.md rule that governs them — [`todos/sync-child-escalation-removal.md`](../todos/sync-child-escalation-removal.md)), which is a code change and therefore its own commit | —                                    | **docs sweep done — coverage and the escalation-code deletion owed** |
