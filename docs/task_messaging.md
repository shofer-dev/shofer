...
repo's docs-hygiene check. **Live-verified on staging (2026-08-28) on every
...
| **6** | sweep and close: the retired vocabulary recorded in the docs-hygiene check and every live-sounding mention of it rewritten across the corpus, downstream repos' own references noted on their own cadence, and the CHANGELOG entry — all **done**. The now-unreachable sync-child escalation machinery (`escalateFollowupToConversation`, the conversation-driver probe, the subscriber census and the AGENTS.md rule that governed them) is deleted. The `integration-tests/` coverage each verified plane earns is **written**: slices 49-52 (in-process, pub/sub, A2A, Temporal), one `@heavy` file each, driven through the AgentApi because no product endpoint serves a mailbox | —                                    | **done**                                        |
