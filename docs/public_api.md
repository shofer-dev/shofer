...
| [`deleteQueuedMessage`](../src/extension/api.ts:270)      | `(messageId: string) => void`                            | Removes a queued message by ID from the current task's message queue. **Note:** this is implemented in the API class but not yet declared in the `ShoferAPI` type interface. |
