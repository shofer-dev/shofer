# RooCode Extension Structure

## Overview

RooCode is a VS Code extension (fork of Cline/Roo-Cline) that provides an AI coding assistant. It uses a pnpm monorepo structure with TypeScript for the extension and React for the webview UI.

**Project root**: `/home/alsterg/Projects/arkware.ai/extensions/Roo-Code/`

## Directory Structure

```
extensions/Roo-Code/
├── src/                          # Main extension source (roo-cline package)
...
│   │   └── webview/
│   │       ├── ClineProvider.ts  # Main provider - manages tasks, state, webview
...
### ClineProvider.ts (`src/core/webview/ClineProvider.ts`)
...
─────────────────────────────────────────────────
ClineProvider                    ExtensionStateContext
     │                                  │
...
1. **Task needs input** (TaskInteractive event):
    ...
    - ClineProvider catches event, posts `taskNotification` to webview
...
2. **Task completes** (TaskIdle event):
    ...
    - Emits `tasks:updated` → ClineProvider posts `parallelTasksUpdated`
