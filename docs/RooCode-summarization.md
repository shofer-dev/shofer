# Roo-Code Context Management & Summarization

## Overview

Roo-Code uses a **reactive** approach to context management — condensation/truncation is triggered when approaching context window limits, not proactively on every request.
...
- [`MIN_CONDENSE_THRESHOLD = 5`](extensions/Roo-Code/src/core/condense/index.ts:111) - Minimum user-configurable % of context window for condensation trigger
- [`MAX_CONDENSE_THRESHOLD = 100`](extensions/Roo-Code/src/core/condense/index.ts:112) - Maximum user-configurable %
- Note: [`TOKEN_BUFFER_PERCENTAGE = 0.1`](extensions/Roo-Code/src/core/context-management/index.ts:26) acts as a hard safety net—condensation/truncation fires when tokens exceed 90% of context window minus output reservation
- Default [`autoCondenseContextPercent = 90`](extensions/Roo-Code/src/core/task/Task.ts:4460) per the Task state
...
1. **Dual Trigger Mechanism** (either condition fires condensation):
    ...
    The safety net is hardcoded via [`TOKEN_BUFFER_PERCENTAGE = 0.1`](extensions/Roo-Code/src/core/context-management/index.ts:26) — meaning condensation **will always trigger by ~90% utilization** even if the user-configured percentage threshold is set higher (e.g., 100%). The percentage threshold is user-configurable between [`MIN_CONDENSE_THRESHOLD (5%)`](extensions/Roo-Code/src/core/condense/index.ts:111) and [`MAX_CONDENSE_THRESHOLD (100%)`](extensions/Roo-Code/src/core/condense/index.ts:112).

2. **Invocation point**: [`manageContext()`](extensions/Roo-Code/src/core/context-management/index.ts:255) is called at the start of every API request in [`Task.attemptApiRequest()`](extensions/Roo-Code/src/core/task/Task.ts:4580), before sending messages to the model.

3. **Process**:

    - Extracts messages since last summary via [`getMessagesSinceLastSummary()`](extensions/Roo-Code/src/core/condense/index.ts:525)
    - Converts tool_use/tool_result blocks to text (no tools param needed for summarization call)
    - Calls LLM with [`SUMMARY_PROMPT`](extensions/Roo-Code/src/core/condense/index.ts:114)
...
- Types: `packages/types/src/message.ts` (ClineMessage, contextCondense, contextTruncation)
