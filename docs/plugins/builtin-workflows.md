...
> **✅ Shipped** — as the bundled **`builtin-workflows` plugin**
> ([`plugins/builtin-workflows/`](../../plugins/builtin-workflows/)), enabled by
> default. The two `.slang` sources are a `contributes.workflows` contribution, not
> files inside the extension bundle; an organization suppresses them by suppressing
> the plugin (see [Governance](#governance)).
...
| --- | ------------------- | ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `debug`             | Collaborative Debug | Two developers independently triage, converge on root cause through peer review, get user sign-off, then one fixes and the other reviews.                           | `bug`    | [`plugins/builtin-workflows/workflows/debug.slang`](../../plugins/builtin-workflows/workflows/debug.slang)                         |
| 2   | `implement-feature` | Implement a Feature | Multi-agent feature implementation pipeline. Architect orchestrates exploration, design approval, implementation, and review with Developer + Reviewer specialists. | `wrench` | [`plugins/builtin-workflows/workflows/implement-feature.slang`](../../plugins/builtin-workflows/workflows/implement-feature.slang) |

### 2.1 File Locations & Override Precedence

| Priority    | Directory                                                                            | Scope                        |
| ----------- | ------------------------------------------------------------------------------------ | ---------------------------- |
| 1 (lowest)  | [`plugins/builtin-workflows/workflows/`](../../plugins/builtin-workflows/workflows/) | Plugin-contributed (shipped) |
...
1. **Plugin** — the `workflows/` dir of every enabled plugin that declares `contributes.workflows`, read through `PluginManager.getContributedWorkflowDirs()`
...
- When the flag is truthy, `discoverWorkflows()` **skips the built-in layer**
  (the plugin is not loaded) and returns only the global + project workflows;
...
| File                                                                                                                               | Role                                                                                                                  |
| ---------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| [`plugins/builtin-workflows/workflows/debug.slang`](../../plugins/builtin-workflows/workflows/debug.slang)                         | Built-in Debug workflow `.slang` source                                                                               |
| [`plugins/builtin-workflows/workflows/implement-feature.slang`](../../plugins/builtin-workflows/workflows/implement-feature.slang) | Built-in Implement a Feature workflow `.slang` source                                                                 |
...
| [`packages/core/src/workflow/wait-for-task-helper.ts`](../../src/core/workflow/wait-for-task-helper.ts)                            | Shared event-driven wait helper (used by both `WaitForTaskTool` and `WorkflowTask.waitForStakes`)                     |
...
| [`docs/plugins/builtin-modes.md`](./builtin-modes.md)                                                                              | Built-in modes SoT — how agent Tasks get their system prompt, tools, and API configuration                            |
