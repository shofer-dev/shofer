...
> **Scope differs by surface.** In the **standalone `.slang` editor** the topology renders the _full_ static graph (every agent + every declared `stake`/`await`/`peer` edge) — the map of what the flow _could_ do. In the **WorkflowView** there is no live Topology tab; the current-round topology appears inline in Events instead (see below). Before the run starts (or once converged) nothing is active. See [`currentRoundSubset()`](../src/core/webview/slang-render.js).

#### Inline topology in Events (WorkflowView)

WorkflowView has no Topology tab. Instead, each round's headline line in the **Events** feed is followed by a compact **Mermaid** snapshot of that round's topology, emitted by [`topologyToMermaid()`](../src/core/workflow/slang-types.ts) and rendered by the existing `MarkdownBlock` → `MermaidBlock` pipeline. It mirrors the live edge logic of [`topologyCurrentEdges()`](../src/core/webview/slang-render.js): `running` agents draw outbound `stake` edges (solid) to their `sendingTo` targets, `blocked` agents draw inbound `await` edges (dashed) from their `waitingFor` sources, and node colors match [`agentStatusColor()`](../src/core/webview/slang-render.js) (running green, blocked purple, committed gray, error red, idle blue). It is emitted **after** `dispatchStakes()` (in [`WorkflowTask.ts`](../src/core/workflow/WorkflowTask.ts)) so `status="running"`/`sendingTo` are already set — the same live state the old Topology tab showed.
