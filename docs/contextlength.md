...
1. **[`vscode-llm.ts`](extensions/shofer/packages/types/src/providers/vscode-llm.ts)** — emptied static map, kept type compatibility; later extended `VsCodeLmChatInfo` with `capabilities`, `arkwareCapabilities`, and `arkwarePricing` fields populated from the side channel.
...
5. **[`vscode-lm.ts`](extensions/shofer/src/api/providers/vscode-lm.ts)** — runtime handler removed the `openAiModelInfoSaneDefaults.contextWindow` (128K) fallback and the hardcoded `supportsImages: false` / `supportsPromptCache: true` flags; all three now come from `this.client.maxInputTokens` and `arkwareCapabilities` (fetched via `arkware.llm.getModelCapabilities`).
