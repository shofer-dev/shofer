...
| **Timeline rewind / pending-edit replay**             | operate by `ts` and array index            | ✅ Host holds the full in-memory array; webview addresses messages by `ts`, never by absolute index                                                                                                                                                                                                  |
...
timeline rewind, LLM context packing, etc. The optimization is purely on the
