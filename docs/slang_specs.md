...
| ------------------- | -------- | ----------- | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `mode`              | no\*     | ✅ wired    | string                  | Shofer mode slug. Shofer extension (`mode: "code"`). Selects the agent Task's **tool set** and, through the host's per-mode API-configuration association (`modeApiConfigs`), the **provider profile** its LLM calls go to — so two agents in one flow naming two modes may legitimately run on two different models. Stored on `AgentMeta.mode`. |
...
\* `mode` is optional in the grammar. The in-editor backend defaults an omitted
one to `defaultModeSlug` (`"code"`) at spawn; a runner driving the agent over the
plugin API sends no mode at all in that case, leaving the node on whatever its
own configuration activated. Either way a mode that is **named but not defined**
is an error, never a fallback — the host refuses the spawn rather than handing
back a different agent, on a different model, reporting success.
