...
| Capability              | OpenCode                                                                                                                                                                                                                                                                                       | Shofer                                                                                                                                                                                                                                                                                                          |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Orchestration model** | Custom agent topologies are defined **declaratively** in Markdown files (`.opencode/agents/`, `~/.config/opencode/agents/`) with per-agent tool-permission locking (e.g. Plan mode confirms every shell command, Build mode runs autonomously); routing between them is imperative at runtime. | Routing is imperative here too, but each subagent is a first-class task in a local tree with its own mode, provider, transcript and cost, and the **Sequence** tab visualizes the real spawn/message/await traffic. A caller can additionally pin a subtask's result to a JSON Schema, enforced at decode time. |
| **Parallelism**         | The TUI session feels linear, but the engine spins up specialized subagents (general, explore, scout) as parallel, isolated background tasks.                                                                                                                                                  | Many concurrent tasks and deep subtask trees, with background subtasks, async MCP calls, and least-privilege inter-agent peer messaging.                                                                                                                                                                        |
...
| **Observable multi-agent runs**    | You want every subagent to be an inspectable task — its own transcript, mode, model and cost — not an opaque hop.     |
