...
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Task & agent visibility** | Terminal-first, with `/agents` and `/tasks` for background job management, and a companion Desktop app for parallel tasks, visual diffs, server previews, and PR status. | A native VS Code cockpit: a graphical task tree plus per-task **Topology / Sequence / Swimlane** diagrams, a **Stats** tab (active-time donut + per-tool breakdown aggregated across the whole tree), and a filterable **Logs** tab — the agent graph and message routing are visualized live, in-editor. |
...
| **Cross-session memory**      | **Auto Memory** autonomously persists learnings, build commands, and debugging insights across sessions (beyond static `CLAUDE.md`).                                                                                         | A persistent **Assistant Agent** holds a long-lived, shared context window other tasks query via `ask_assistant_agent` — pairing learned memory with the semantic index above so many cheap queries reuse one pre-warmed context instead of re-searching.                                             |
